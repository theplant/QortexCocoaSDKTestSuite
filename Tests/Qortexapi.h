// Generated by github.com/hypermusk/hypermusk
// DO NOT EDIT

#import <Foundation/Foundation.h>


@interface QXQortexapi : NSObject
@property (nonatomic, strong) NSString * BaseURL;
@property (nonatomic, assign) BOOL Verbose;
@property (nonatomic, assign) NSTimeInterval RequestTimeoutInterval;
+ (QXQortexapi *) get;

@end



// --- TaskSelectorItem ---
@interface QXTaskSelectorItem : NSObject

@property (nonatomic, strong) NSNumber * StoreKey;
@property (nonatomic, strong) NSString * DisplayText;
@property (nonatomic, assign) BOOL IsCurrent;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Blog ---
@interface QXBlog : NSObject

@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Description;
@property (nonatomic, strong) NSString * SideContent;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EmbedOrg ---
@interface QXEmbedOrg : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * LogoURL;
@property (nonatomic, assign) BOOL NoNeedToShare;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EmbedUser ---
@interface QXEmbedUser : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Avatar;
@property (nonatomic, strong) NSString * JID;
@property (nonatomic, strong) NSString * Timezone;
@property (nonatomic, strong) NSString * OrgName;
@property (nonatomic, assign) BOOL IsSuperUser;
@property (nonatomic, assign) BOOL IsShare;
@property (nonatomic, strong) NSString * OrganizationId;
@property (nonatomic, strong) NSString * OriginalOrgId;
@property (nonatomic, strong) NSString * ProfileURL;
@property (nonatomic, assign) BOOL NoDetail;
@property (nonatomic, assign) BOOL UnfollowSharedGroup;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- PanelStatus ---
@interface QXPanelStatus : NSObject

@property (nonatomic, assign) BOOL HasDraft;
@property (nonatomic, assign) BOOL HasWatchList;
@property (nonatomic, assign) BOOL HasChat;
@property (nonatomic, strong) NSNumber * ShowMarkUnreadThreshold;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OrgSettings ---
@interface QXOrgSettings : NSObject

@property (nonatomic, assign) BOOL AllowUsersCreateGroups;
@property (nonatomic, assign) BOOL AllowUsersInvitePeople;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TagIndex ---
@interface QXTagIndex : NSObject

@property (nonatomic, strong) NSString * Tag;
@property (nonatomic, strong) NSNumber * Index;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EmbedGroup ---
@interface QXEmbedGroup : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * IconName;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSString * TaskLink;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupSelectorItem ---
@interface QXGroupSelectorItem : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, assign) BOOL IsSelected;
@property (nonatomic, assign) BOOL Accessible;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Attachment ---
@interface QXAttachment : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSArray * OwnerId;
@property (nonatomic, strong) NSString * Category;
@property (nonatomic, strong) NSString * Filename;
@property (nonatomic, strong) NSString * ShortFilename;
@property (nonatomic, strong) NSString * ContentType;
@property (nonatomic, strong) NSString * ContentId;
@property (nonatomic, strong) NSString * MD5;
@property (nonatomic, strong) NSNumber * ContentLength;
@property (nonatomic, strong) NSString * Error;
@property (nonatomic, strong) NSArray * GroupId;
@property (nonatomic, strong) NSDate * UploadTime;
@property (nonatomic, strong) NSNumber * Width;
@property (nonatomic, strong) NSNumber * Height;
@property (nonatomic, strong) NSString * URL;
@property (nonatomic, strong) NSString * S1ThumbURL;
@property (nonatomic, strong) NSString * MThumbURL;
@property (nonatomic, strong) NSString * LThumbURL;
@property (nonatomic, strong) NSString * ImageIconURL;
@property (nonatomic, strong) NSString * FileIconURL;
@property (nonatomic, strong) NSString * HumanSize;
@property (nonatomic, assign) BOOL IsImage;
@property (nonatomic, strong) NSString * FileKind;
@property (nonatomic, strong) NSNumber * CrocodocStatus;
@property (nonatomic, strong) NSString * SessionId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- AssignableUser ---
@interface QXAssignableUser : NSObject

@property (nonatomic, strong) NSString * UserId;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, assign) BOOL IsAssigned;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TimeTrackingItem ---
@interface QXTimeTrackingItem : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * UserName;
@property (nonatomic, strong) NSString * FinishDate;
@property (nonatomic, strong) NSNumber * SpentTime;
@property (nonatomic, strong) NSString * TimeUnit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- LinkedEntry ---
@interface QXLinkedEntry : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * EType;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AuthorId;
@property (nonatomic, assign) BOOL IsRoot;
@property (nonatomic, strong) NSString * RootId;
@property (nonatomic, strong) NSString * RootEntryTitle;
@property (nonatomic, strong) NSString * Link;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- BasedOnPost ---
@interface QXBasedOnPost : NSObject

@property (nonatomic, strong) NSString * RootHtmlTitle;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, assign) BOOL IsComment;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupCount ---
@interface QXGroupCount : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSNumber * UnreadCount;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupHeader ---
@interface QXGroupHeader : NSObject

@property (nonatomic, assign) BOOL HasToFollow;
@property (nonatomic, assign) BOOL IsFollowing;
@property (nonatomic, assign) BOOL IsManaging;
@property (nonatomic, assign) BOOL HasFileTab;
@property (nonatomic, assign) BOOL HasToDoTab;
@property (nonatomic, assign) BOOL IsQortexSupport;
@property (nonatomic, assign) BOOL SelectedGroup;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- InlineHelp ---
@interface QXInlineHelp : NSObject

@property (nonatomic, assign) BOOL QortexOverview;
@property (nonatomic, assign) BOOL WhatNext;
@property (nonatomic, assign) BOOL WhatChats;
@property (nonatomic, strong) NSString * InviteOthersURL;
@property (nonatomic, strong) NSString * WhatChatsURL;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EmailChanger ---
@interface QXEmailChanger : NSObject

@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * SharingToken;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Newsletter ---
@interface QXNewsletter : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ContactInfo ---
@interface QXContactInfo : NSObject

@property (nonatomic, strong) NSString * FirstName;
@property (nonatomic, strong) NSString * LastName;
@property (nonatomic, strong) NSString * CompanyName;
@property (nonatomic, strong) NSString * CompanySize;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Phone;
@property (nonatomic, strong) NSString * Country;
@property (nonatomic, strong) NSString * City;
@property (nonatomic, strong) NSString * HelpContent;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TotalStats ---
@interface QXTotalStats : NSObject

@property (nonatomic, strong) NSNumber * OrgCount;
@property (nonatomic, strong) NSNumber * MemberCount;
@property (nonatomic, strong) NSNumber * GroupCount;
@property (nonatomic, strong) NSNumber * EntryCount;
@property (nonatomic, strong) NSNumber * CommentCount;
@property (nonatomic, strong) NSNumber * ChatCount;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- AccessReq ---
@interface QXAccessReq : NSObject

@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * PriorityCode;
@property (nonatomic, strong) NSString * AccessCode;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSString * ApprovedBy;
@property (nonatomic, strong) NSString * CreatedAt;
@property (nonatomic, strong) NSString * UpdatedAt;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- InnerMessage ---
@interface QXInnerMessage : NSObject

@property (nonatomic, strong) NSString * ByUser;
@property (nonatomic, strong) NSString * GroupName;
@property (nonatomic, strong) NSString * GroupLink;
@property (nonatomic, strong) NSString * OrgName;
@property (nonatomic, assign) BOOL IsDeletedGroupMessage;
@property (nonatomic, assign) BOOL IsCreatedGroupMessage;
@property (nonatomic, assign) BOOL IsSetupOrgMessage;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OrgUnreadInfo ---
@interface QXOrgUnreadInfo : NSObject

@property (nonatomic, strong) NSString * OrgId;
@property (nonatomic, strong) NSNumber * FeedUnreadCount;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MarketableMemberInfo ---
@interface QXMarketableMemberInfo : NSObject

@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * FirstName;
@property (nonatomic, strong) NSString * LastName;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSString * InvitOrg;
@property (nonatomic, strong) NSString * GotoURL;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ToDoCSVItem ---
@interface QXToDoCSVItem : NSObject

@property (nonatomic, strong) NSString * Creator;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSString * EstimateTime;
@property (nonatomic, strong) NSString * TotalSpentTime;
@property (nonatomic, strong) NSString * TimeUnit;
@property (nonatomic, strong) NSString * SpentTimeDetail;
@property (nonatomic, strong) NSString * CreateTime;
@property (nonatomic, strong) NSString * UpdateTime;
@property (nonatomic, strong) NSString * Due;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TranslatedComment ---
@interface QXTranslatedComment : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Content;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- SupportedLanguage ---
@interface QXSupportedLanguage : NSObject

@property (nonatomic, strong) NSString * StoreKey;
@property (nonatomic, strong) NSString * DisplayText;
@property (nonatomic, assign) BOOL IsCurrent;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- SelectionTextInFo ---
@interface QXSelectionTextInFo : NSObject

@property (nonatomic, strong) NSString * Text;
@property (nonatomic, strong) NSNumber * Occured;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- BroadcastInput ---
@interface QXBroadcastInput : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSArray * ToOrgIds;
@property (nonatomic, strong) NSString * BroadcastType;
@property (nonatomic, strong) NSString * RootId;
@property (nonatomic, strong) NSString * LocaleName;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QortexSupportInput ---
@interface QXQortexSupportInput : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSArray * ToOrgIds;
@property (nonatomic, strong) NSString * RootId;
@property (nonatomic, strong) NSString * Audiance;
@property (nonatomic, assign) BOOL KnowledgeBase;
@property (nonatomic, assign) BOOL InlineHelp;
@property (nonatomic, assign) BOOL PublishedToUsers;
@property (nonatomic, strong) NSString * LinkTitle;
@property (nonatomic, strong) NSString * LocaleName;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupInput ---
@interface QXGroupInput : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Description;
@property (nonatomic, strong) NSString * LogoURL;
@property (nonatomic, strong) NSString * IconName;
@property (nonatomic, strong) NSString * Slug;
@property (nonatomic, assign) BOOL IsPrivate;
@property (nonatomic, assign) BOOL IsShared;
@property (nonatomic, strong) NSArray * GroupOwners;
@property (nonatomic, strong) NSArray * InvitedOrgIds;
@property (nonatomic, assign) BOOL AutoGenSlug;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OrgSettingsInput ---
@interface QXOrgSettingsInput : NSObject

@property (nonatomic, assign) BOOL AllowUsersCreateGroups;
@property (nonatomic, assign) BOOL AllowUsersInvitePeople;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OrganizationInput ---
@interface QXOrganizationInput : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Summary;
@property (nonatomic, strong) NSString * Address;
@property (nonatomic, strong) NSString * Phone;
@property (nonatomic, strong) NSString * Website;
@property (nonatomic, strong) NSString * Country;
@property (nonatomic, strong) NSString * Size;
@property (nonatomic, strong) NSArray * Domains;
@property (nonatomic, strong) NSString * QortexURL;
@property (nonatomic, strong) NSString * LogoURL;
@property (nonatomic, strong) NSString * SharingToken;
@property (nonatomic, strong) NSString * ContactWay;
@property (nonatomic, assign) BOOL NeedDemo;
@property (nonatomic, assign) BOOL RestrictSubscriptionMail;
@property (nonatomic, assign) BOOL AnyoneCanJoin;
@property (nonatomic, strong) NSArray * LanguageCodes;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- LikeInput ---
@interface QXLikeInput : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * Like;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- PreferencesInput ---
@interface QXPreferencesInput : NSObject

@property (nonatomic, strong) NSString * Timezone;
@property (nonatomic, strong) NSString * TimezoneOffset;
@property (nonatomic, strong) NSString * PreferFullName;
@property (nonatomic, strong) NSString * EnterForNewLine;
@property (nonatomic, strong) NSString * AsideGroupsCollapse;
@property (nonatomic, strong) NSString * AsideOtherGroupsCollapse;
@property (nonatomic, strong) NSString * ShowMarkUnreadThreshold;
@property (nonatomic, strong) NSString * AdminModeOn;
@property (nonatomic, strong) NSString * PreferMarkdown;
@property (nonatomic, strong) NSString * AutoFollowPublicGroup;
@property (nonatomic, strong) NSString * EnableHTML5Notification;
@property (nonatomic, strong) NSString * UserLocationCityName;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MemberAccountInput ---
@interface QXMemberAccountInput : NSObject

@property (nonatomic, strong) NSString * FirstName;
@property (nonatomic, strong) NSString * LastName;
@property (nonatomic, strong) NSString * AvatarURL;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- NewsletterInput ---
@interface QXNewsletterInput : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ShareChatInput ---
@interface QXShareChatInput : NSObject

@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSString * BasedConvId;
@property (nonatomic, strong) NSString * BaseOnEntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ContactInput ---
@interface QXContactInput : NSObject

@property (nonatomic, strong) NSString * FirstName;
@property (nonatomic, strong) NSString * LastName;
@property (nonatomic, strong) NSString * CompanyName;
@property (nonatomic, strong) NSString * CompanySize;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Phone;
@property (nonatomic, strong) NSString * Country;
@property (nonatomic, strong) NSString * City;
@property (nonatomic, strong) NSString * HelpContent;
@property (nonatomic, assign) BOOL Fake;
@property (nonatomic, assign) BOOL IsAgreed;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- UserProfileInput ---
@interface QXUserProfileInput : NSObject

@property (nonatomic, strong) NSString * Summary;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Department;
@property (nonatomic, strong) NSString * Location;
@property (nonatomic, strong) NSString * Expertise;
@property (nonatomic, strong) NSString * Interests;
@property (nonatomic, strong) NSString * BirthMonth;
@property (nonatomic, strong) NSString * BirthDay;
@property (nonatomic, strong) NSString * WorkPhone;
@property (nonatomic, strong) NSString * Mobile;
@property (nonatomic, strong) NSString * Twitter;
@property (nonatomic, strong) NSString * Skype;
@property (nonatomic, strong) NSString * Facebook;
@property (nonatomic, strong) NSArray * OtherWebsites;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MailUpdatesInput ---
@interface QXMailUpdatesInput : NSObject

@property (nonatomic, assign) BOOL IndividualIsOn;
@property (nonatomic, strong) NSNumber * SendLag;
@property (nonatomic, assign) BOOL AckRequest;
@property (nonatomic, assign) BOOL AckConfirmation;
@property (nonatomic, assign) BOOL Todo;
@property (nonatomic, assign) BOOL TodoConfirmation;
@property (nonatomic, assign) BOOL SystemMessage;
@property (nonatomic, assign) BOOL EntryNotification;
@property (nonatomic, assign) BOOL Like;
@property (nonatomic, assign) BOOL SendTimeIsOn;
@property (nonatomic, assign) BOOL Mon;
@property (nonatomic, assign) BOOL Tue;
@property (nonatomic, assign) BOOL Wed;
@property (nonatomic, assign) BOOL Thu;
@property (nonatomic, assign) BOOL Fri;
@property (nonatomic, assign) BOOL Sat;
@property (nonatomic, assign) BOOL Sun;
@property (nonatomic, assign) BOOL SendHoursIsOn;
@property (nonatomic, strong) NSNumber * StartAt;
@property (nonatomic, strong) NSNumber * EndAt;
@property (nonatomic, assign) BOOL DailyIsOn;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MailPreferenceInput ---
@interface QXMailPreferenceInput : NSObject

@property (nonatomic, assign) BOOL Expecting;
@property (nonatomic, strong) NSNumber * SendInterval;
@property (nonatomic, strong) NSNumber * SendLag;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TaskInput ---
@interface QXTaskInput : NSObject

@property (nonatomic, strong) NSString * TaskId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AssigneeId;
@property (nonatomic, strong) NSNumber * TodoStatus;
@property (nonatomic, strong) NSNumber * Label;
@property (nonatomic, strong) NSString * EstimateTime;
@property (nonatomic, strong) NSString * SpentTime;
@property (nonatomic, assign) BOOL IsClaiming;
@property (nonatomic, strong) NSString * ToUserIds;
@property (nonatomic, strong) NSString * TimetrackingHistoryUpdate;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TaskPwMap ---
@interface QXTaskPwMap : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSNumber * PriorityWeight;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MyCount ---
@interface QXMyCount : NSObject

@property (nonatomic, strong) NSString * UserId;
@property (nonatomic, strong) NSNumber * FollowedUnreadCount;
@property (nonatomic, strong) NSNumber * NotificationUnreadCount;
@property (nonatomic, strong) NSNumber * ActiveTasksCount;
@property (nonatomic, strong) NSNumber * OfflineMessageCount;
@property (nonatomic, strong) NSArray * GroupCounts;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TaskOutline ---
@interface QXTaskOutline : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * EntryTitle;
@property (nonatomic, strong) NSString * EntryLink;
@property (nonatomic, assign) BOOL IsComment;
@property (nonatomic, strong) QXEmbedUser * Assignee;
@property (nonatomic, strong) NSString * AuthorName;
@property (nonatomic, strong) QXEmbedGroup * Group;
@property (nonatomic, strong) NSString * Age;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSNumber * StatusCode;
@property (nonatomic, strong) NSString * Due;
@property (nonatomic, strong) NSString * Label;
@property (nonatomic, strong) NSNumber * LabelCode;
@property (nonatomic, strong) NSString * EstimateTime;
@property (nonatomic, strong) NSNumber * EstimateTimeFloat64;
@property (nonatomic, strong) NSString * SpentTime;
@property (nonatomic, strong) NSNumber * PriorityWeight;
@property (nonatomic, strong) NSNumber * Priority;
@property (nonatomic, strong) NSString * CompleteAtStr;
@property (nonatomic, strong) NSNumber * CompleteAtUnixNano;
@property (nonatomic, assign) BOOL IsTitleCreatedBy;
@property (nonatomic, assign) BOOL ActionNeeded;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- BlogEntry ---
@interface QXBlogEntry : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * HtmlTitle;
@property (nonatomic, strong) NSString * Slug;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSString * LocalCreatedAt;
@property (nonatomic, strong) NSString * Permalink;
@property (nonatomic, strong) NSString * CreateCommentURL;
@property (nonatomic, strong) NSString * HtmlContent;
@property (nonatomic, strong) NSString * HtmlContentPart;
@property (nonatomic, strong) QXEmbedUser * Author;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupUsers ---
@interface QXGroupUsers : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSArray * EmbedUsers;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Invitation ---
@interface QXInvitation : NSObject

@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * SentAgo;
@property (nonatomic, strong) QXEmbedUser * ByUser;
@property (nonatomic, assign) BOOL HideInPendingList;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- AbandonInfo ---
@interface QXAbandonInfo : NSObject

@property (nonatomic, strong) QXEmbedOrg * AbandonFromOrg;
@property (nonatomic, strong) NSArray * AvailableOrgs;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupSelector ---
@interface QXGroupSelector : NSObject

@property (nonatomic, strong) NSString * Header;
@property (nonatomic, strong) NSString * SelectedGroupId;
@property (nonatomic, strong) QXGroupSelectorItem * SysMessage;
@property (nonatomic, strong) NSArray * FollowingNormalGroups;
@property (nonatomic, strong) NSArray * FollowingSharedGroups;
@property (nonatomic, strong) NSArray * UnFollowingNormalGroups;
@property (nonatomic, strong) NSArray * UnFollowingSharedGroups;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TimeTrackingUpdateLog ---
@interface QXTimeTrackingUpdateLog : NSObject

@property (nonatomic, assign) BOOL IsUpdateLog;
@property (nonatomic, assign) BOOL IsDeleteLog;
@property (nonatomic, assign) BOOL IsSelfUpdate;
@property (nonatomic, strong) NSNumber * OldSpentTime;
@property (nonatomic, strong) NSNumber * NewSpentTime;
@property (nonatomic, strong) QXEmbedUser * Owner;
@property (nonatomic, strong) QXEmbedUser * Updater;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- LanguageSelector ---
@interface QXLanguageSelector : NSObject

@property (nonatomic, assign) BOOL IsFirst;
@property (nonatomic, strong) NSString * Index;
@property (nonatomic, strong) NSArray * SupportedLanguages;
@property (nonatomic, strong) NSArray * PreferredLanguages;
@property (nonatomic, strong) NSArray * RestLanguages;
@property (nonatomic, strong) NSArray * UILanguages;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- AdvancedToDoSettings ---
@interface QXAdvancedToDoSettings : NSObject

@property (nonatomic, assign) BOOL Enabled;
@property (nonatomic, assign) BOOL EnableTimeEstimate;
@property (nonatomic, assign) BOOL EnableTimeTracking;
@property (nonatomic, strong) NSNumber * TimeUnit;
@property (nonatomic, strong) QXEmbedUser * ProjectManager;
@property (nonatomic, strong) NSArray * Labels;
@property (nonatomic, strong) NSArray * NotYetOpenStatuses;
@property (nonatomic, strong) NSArray * OpenStatuses;
@property (nonatomic, strong) NSArray * ClosedStatuses;
@property (nonatomic, strong) QXTagIndex * DefaultNotYetOpenStatus;
@property (nonatomic, strong) QXTagIndex * DefaultOpenStatus;
@property (nonatomic, strong) QXTagIndex * DefaultClosedStatus;
@property (nonatomic, strong) NSNumber * LabelCounter;
@property (nonatomic, strong) NSNumber * NotYetOpenStatusCounter;
@property (nonatomic, strong) NSNumber * OpenStatusCounter;
@property (nonatomic, strong) NSNumber * ClosedStatusCounter;
@property (nonatomic, strong) NSArray * EnabledSharingOrgs;
@property (nonatomic, assign) BOOL ShowedThrowawayStatusSuggestion;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TranslatedThread ---
@interface QXTranslatedThread : NSObject

@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSArray * Comments;
@property (nonatomic, assign) BOOL IsCommentOnly;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EntryVersion ---
@interface QXEntryVersion : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSDate * UpdatedAt;
@property (nonatomic, strong) NSString * LocalUpdatedAt;
@property (nonatomic, strong) NSString * UpdatedAtUnixNano;
@property (nonatomic, strong) QXEmbedUser * CurrentVersionEditor;
@property (nonatomic, assign) BOOL IsNewVersion;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Request ---
@interface QXRequest : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * CurrentPrefixURL;
@property (nonatomic, strong) NSString * Info;
@property (nonatomic, strong) NSString * ActionButton;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) QXEmbedOrg * ToOrg;
@property (nonatomic, strong) QXEmbedGroup * SharedGroup;
@property (nonatomic, strong) NSString * SharedOrgIdHex;
@property (nonatomic, strong) NSString * FromUserIdHex;
@property (nonatomic, strong) QXEmbedUser * SharedInvitee;
@property (nonatomic, strong) QXEmbedUser * SharedInviter;
@property (nonatomic, strong) QXEmbedUser * SharedResponsor;
@property (nonatomic, strong) NSString * ToEmail;
@property (nonatomic, strong) NSString * State;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ShareRequest ---
@interface QXShareRequest : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) QXEmbedUser * FromUser;
@property (nonatomic, strong) QXEmbedUser * ToUser;
@property (nonatomic, strong) QXEmbedUser * Responser;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) QXEmbedOrg * ToOrg;
@property (nonatomic, strong) NSArray * JoinedOrgs;
@property (nonatomic, strong) QXEmbedGroup * SharedGroup;
@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * ToEmail;
@property (nonatomic, strong) NSString * PendingDuration;
@property (nonatomic, assign) BOOL IsNewAccount;
@property (nonatomic, assign) BOOL IsPending;
@property (nonatomic, assign) BOOL IsAccepted;
@property (nonatomic, assign) BOOL IsRejected;
@property (nonatomic, assign) BOOL IsForwarded;
@property (nonatomic, assign) BOOL IsCanceled;
@property (nonatomic, assign) BOOL IsStopped;
@property (nonatomic, strong) NSString * Info;
@property (nonatomic, strong) NSString * ActionButton;
@property (nonatomic, strong) NSString * RequestBarHtml;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupSharingInfo ---
@interface QXGroupSharingInfo : NSObject

@property (nonatomic, assign) BOOL IsSharing;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) NSArray * AccpetedOrgs;
@property (nonatomic, strong) NSArray * ForwardedOrgs;
@property (nonatomic, strong) NSArray * PendingToEmails;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Message ---
@interface QXMessage : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * ConversationId;
@property (nonatomic, strong) NSString * UserId;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSString * HtmlContent;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) QXEmbedUser * EmbedUser;
@property (nonatomic, assign) BOOL ShowUser;
@property (nonatomic, assign) BOOL IsOffline;
@property (nonatomic, strong) NSString * HighlightedContent;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EntryInput ---
@interface QXEntryInput : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * EType;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * IsToGroup;
@property (nonatomic, strong) NSString * ToUserIds;
@property (nonatomic, strong) NSString * TodoUserIds;
@property (nonatomic, strong) NSString * MentionedUserIds;
@property (nonatomic, assign) BOOL IsAcknowledgement;
@property (nonatomic, assign) BOOL IsToDo;
@property (nonatomic, strong) NSString * TaskDue;
@property (nonatomic, strong) NSNumber * TodoStatus;
@property (nonatomic, strong) NSNumber * Priority;
@property (nonatomic, strong) NSNumber * Label;
@property (nonatomic, strong) NSString * EstimateTime;
@property (nonatomic, strong) NSString * RootId;
@property (nonatomic, assign) BOOL NewVersion;
@property (nonatomic, strong) NSString * OldGroupId;
@property (nonatomic, strong) NSString * LastUpdateAt;
@property (nonatomic, assign) BOOL KnowledgeBase;
@property (nonatomic, assign) BOOL AnyoneCanEdit;
@property (nonatomic, assign) BOOL Presentation;
@property (nonatomic, assign) BOOL IsFromEmail;
@property (nonatomic, assign) BOOL IsPublished;
@property (nonatomic, strong) NSString * Slug;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, assign) BOOL InlineHelp;
@property (nonatomic, strong) NSString * LinkTitle;
@property (nonatomic, strong) NSString * BaseOnEntryId;
@property (nonatomic, assign) BOOL PublishedToUsers;
@property (nonatomic, strong) NSString * LocaleName;
@property (nonatomic, strong) NSString * BasedPostId;
@property (nonatomic, strong) NSString * GroupIdOfBasedPost;
@property (nonatomic, strong) QXSelectionTextInFo * SelectionTextInFo;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QortexSupport ---
@interface QXQortexSupport : NSObject

@property (nonatomic, strong) NSString * Audience;
@property (nonatomic, assign) BOOL IsToOffical;
@property (nonatomic, assign) BOOL IsToAllUsers;
@property (nonatomic, assign) BOOL IsToAllAdmins;
@property (nonatomic, assign) BOOL IsToOrganizations;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) NSArray * ToOrgs;
@property (nonatomic, strong) NSString * ToOrgsHtml;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- EmbedEntry ---
@interface QXEmbedEntry : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * GroupName;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * HtmlTitle;
@property (nonatomic, strong) NSString * EType;
@property (nonatomic, strong) QXEmbedUser * Author;
@property (nonatomic, strong) NSArray * ToUsers;
@property (nonatomic, strong) NSString * Link;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- RelatedEntry ---
@interface QXRelatedEntry : NSObject

@property (nonatomic, strong) NSString * HtmlTitle;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSString * LocalHumanCreatedAt;
@property (nonatomic, strong) QXEmbedUser * Author;
@property (nonatomic, assign) BOOL IsComment;
@property (nonatomic, assign) BOOL IsEmbedded;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OpenAdvancedToDosBucket ---
@interface QXOpenAdvancedToDosBucket : NSObject

@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) QXAdvancedToDoSettings * ToDoSettings;
@property (nonatomic, strong) NSArray * ToDos;
@property (nonatomic, strong) NSNumber * LenOfToDos;
@property (nonatomic, strong) NSNumber * EstimateTotal;
@property (nonatomic, strong) NSString * EstimateUnit;
@property (nonatomic, assign) BOOL Editable;
@property (nonatomic, strong) NSArray * Followers;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Group ---
@interface QXGroup : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Description;
@property (nonatomic, strong) NSString * GType;
@property (nonatomic, strong) NSString * LogoURL;
@property (nonatomic, strong) NSString * IconName;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSString * TaskLink;
@property (nonatomic, strong) NSString * Slug;
@property (nonatomic, strong) QXEmbedUser * Author;
@property (nonatomic, assign) BOOL IsAdmin;
@property (nonatomic, assign) BOOL IsPrivate;
@property (nonatomic, assign) BOOL Editable;
@property (nonatomic, assign) BOOL Managable;
@property (nonatomic, assign) BOOL Accessible;
@property (nonatomic, assign) BOOL FollowedByMe;
@property (nonatomic, assign) BOOL AdministratedByMe;
@property (nonatomic, assign) BOOL IsPreShared;
@property (nonatomic, assign) BOOL IsShared;
@property (nonatomic, assign) BOOL IsDefaultLogoURL;
@property (nonatomic, strong) NSString * HostOrgName;
@property (nonatomic, assign) BOOL IsDispayHostOrgName;
@property (nonatomic, strong) NSNumber * EntriesCount;
@property (nonatomic, strong) NSNumber * FollowersCount;
@property (nonatomic, assign) BOOL IsAnnouncement;
@property (nonatomic, assign) BOOL IsQortexSupport;
@property (nonatomic, strong) NSArray * GroupOwners;
@property (nonatomic, strong) QXGroupSharingInfo * SharingInfo;
@property (nonatomic, strong) NSString * GroupEmailAddress;
@property (nonatomic, strong) QXAdvancedToDoSettings * ToDoSettings;
@property (nonatomic, strong) NSString * TodoGroupingRoute;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Conversation ---
@interface QXConversation : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSArray * UserIds;
@property (nonatomic, strong) NSArray * Participants;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSDate * EndedAt;
@property (nonatomic, strong) NSString * LocalHumanCreatedAt;
@property (nonatomic, strong) NSString * Topic;
@property (nonatomic, assign) BOOL Private;
@property (nonatomic, assign) BOOL IsClose;
@property (nonatomic, assign) BOOL IsShared;
@property (nonatomic, assign) BOOL HasOfflineMessage;
@property (nonatomic, strong) NSString * OfflineLocalTime;
@property (nonatomic, strong) NSArray * SharedMessageIds;
@property (nonatomic, strong) NSNumber * MessagesCount;
@property (nonatomic, strong) NSArray * Messages;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- BasicOpenToDoOutlines ---
@interface QXBasicOpenToDoOutlines : NSObject

@property (nonatomic, strong) QXEmbedUser * Assignee;
@property (nonatomic, strong) NSArray * TaskOutlines;
@property (nonatomic, assign) BOOL Editable;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- TaskLog ---
@interface QXTaskLog : NSObject

@property (nonatomic, assign) BOOL IsClaimed;
@property (nonatomic, assign) BOOL IsAssigneeChanged;
@property (nonatomic, assign) BOOL IsStatusChanged;
@property (nonatomic, assign) BOOL IsEstimationChanged;
@property (nonatomic, assign) BOOL IsTimeTrackingAdded;
@property (nonatomic, assign) BOOL IsTimeTrackingUpdated;
@property (nonatomic, assign) BOOL IsReopened;
@property (nonatomic, assign) BOOL IsLabelChanged;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSDate * VersionAt;
@property (nonatomic, strong) NSString * LocalHumanCreatedAt;
@property (nonatomic, strong) QXEmbedUser * Author;
@property (nonatomic, strong) QXEmbedUser * Assignee;
@property (nonatomic, strong) QXEmbedUser * OldAssignee;
@property (nonatomic, strong) NSNumber * EstimatedTimeValue;
@property (nonatomic, strong) NSString * TimeUnit;
@property (nonatomic, strong) NSNumber * NewSpentTime;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSString * Label;
@property (nonatomic, strong) NSArray * TimeTrackingUpdateLogs;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- NotificationItem ---
@interface QXNotificationItem : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) QXEmbedUser * ToUser;
@property (nonatomic, strong) QXEmbedEntry * ForEntry;
@property (nonatomic, strong) QXEmbedUser * FromUser;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) QXEmbedEntry * CausedByEntry;
@property (nonatomic, strong) NSDate * NotifiedAt;
@property (nonatomic, strong) NSDate * ReadAt;
@property (nonatomic, assign) BOOL Readed;
@property (nonatomic, strong) NSString * Type;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSString * SharingRequestToEmail;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- LanguageSelectors ---
@interface QXLanguageSelectors : NSObject

@property (nonatomic, strong) NSArray * Selectors;
@property (nonatomic, strong) NSString * LabelText;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupTasksOutline ---
@interface QXGroupTasksOutline : NSObject

@property (nonatomic, strong) QXEmbedGroup * Group;
@property (nonatomic, strong) NSArray * AcksAndPendingToDos;
@property (nonatomic, strong) NSArray * SimpleToDos;
@property (nonatomic, strong) NSArray * OpenToDos;
@property (nonatomic, strong) NSNumber * OpenEstimateTotal;
@property (nonatomic, strong) NSArray * NotStartedToDos;
@property (nonatomic, strong) NSNumber * NotStartedEstimateTotal;
@property (nonatomic, strong) NSString * EstimateUnit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Preferences ---
@interface QXPreferences : NSObject

@property (nonatomic, strong) NSString * Timezone;
@property (nonatomic, strong) NSString * TimezoneOffset;
@property (nonatomic, assign) BOOL PreferFullName;
@property (nonatomic, assign) BOOL EnterForNewLine;
@property (nonatomic, assign) BOOL AsideGroupsCollapse;
@property (nonatomic, assign) BOOL AsideOtherGroupsCollapse;
@property (nonatomic, strong) NSNumber * ShowMarkUnreadThreshold;
@property (nonatomic, assign) BOOL AdminModeOn;
@property (nonatomic, assign) BOOL PreferMarkdown;
@property (nonatomic, assign) BOOL AutoFollowPublicGroup;
@property (nonatomic, assign) BOOL EnableHTML5Notification;
@property (nonatomic, strong) QXLanguageSelectors * PreferredLanguageSelectors;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupAside ---
@interface QXGroupAside : NSObject

@property (nonatomic, assign) BOOL IsMyGroupsCollapse;
@property (nonatomic, assign) BOOL IsOtherGroupsCollapse;
@property (nonatomic, assign) BOOL ShowNewGroupButton;
@property (nonatomic, assign) BOOL HaveOtherGroup;
@property (nonatomic, strong) QXGroup * AnnounGroup;
@property (nonatomic, strong) QXGroup * SMGroup;
@property (nonatomic, strong) NSArray * FollowingNormalGroups;
@property (nonatomic, strong) NSArray * FollowingSharedGroups;
@property (nonatomic, strong) NSArray * UnfollowedNormalGroups;
@property (nonatomic, strong) NSArray * UnfollowedSharedGroups;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Organization ---
@interface QXOrganization : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * QortexURL;
@property (nonatomic, strong) NSString * Summary;
@property (nonatomic, strong) NSString * LogoURL;
@property (nonatomic, strong) NSString * Address;
@property (nonatomic, strong) NSString * Country;
@property (nonatomic, strong) NSString * Phone;
@property (nonatomic, strong) NSString * Website;
@property (nonatomic, strong) NSString * Size;
@property (nonatomic, strong) NSString * SizeText;
@property (nonatomic, strong) NSString * SharingToken;
@property (nonatomic, strong) NSArray * Domains;
@property (nonatomic, assign) BOOL RestrictSubscriptionMail;
@property (nonatomic, assign) BOOL IsActive;
@property (nonatomic, assign) BOOL AnyoneCanJoin;
@property (nonatomic, assign) BOOL NeedDemo;
@property (nonatomic, strong) NSString * ContactWay;
@property (nonatomic, assign) BOOL EnableMultilingual;
@property (nonatomic, strong) QXLanguageSelectors * LanguageSelectors;
@property (nonatomic, strong) NSDictionary * SizeOptions;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OpenAdvancedToDosPage ---
@interface QXOpenAdvancedToDosPage : NSObject

@property (nonatomic, strong) QXEmbedUser * Assignee;
@property (nonatomic, strong) NSArray * ActionNeededToDos;
@property (nonatomic, strong) NSArray * ToDosBuckets;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- ClosedAdvancedToDoOutline ---
@interface QXClosedAdvancedToDoOutline : NSObject

@property (nonatomic, strong) QXTagIndex * Status;
@property (nonatomic, strong) NSNumber * Count;
@property (nonatomic, assign) BOOL WithLoadMoreLink;
@property (nonatomic, strong) NSArray * Tasks;
@property (nonatomic, strong) QXGroup * Group;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MyTask ---
@interface QXMyTask : NSObject

@property (nonatomic, strong) NSString * PrefixURL;
@property (nonatomic, strong) NSArray * NeedActionTasks;
@property (nonatomic, strong) NSArray * GroupTasks;
@property (nonatomic, strong) NSArray * ClosedTasks;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MyNotifications ---
@interface QXMyNotifications : NSObject

@property (nonatomic, strong) NSArray * NotificationItems;
@property (nonatomic, assign) BOOL HasMore;
@property (nonatomic, strong) NSNumber * LatestNotifyTime;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- AdvancedTask ---
@interface QXAdvancedTask : NSObject

@property (nonatomic, strong) QXEmbedUser * CurrentAssignee;
@property (nonatomic, assign) BOOL IsTimeEstimationEnabled;
@property (nonatomic, assign) BOOL IsTimeTrackingEnabled;
@property (nonatomic, assign) BOOL IsPendingEstimation;
@property (nonatomic, strong) NSNumber * LabelCode;
@property (nonatomic, strong) NSString * Label;
@property (nonatomic, strong) NSNumber * StatusCode;
@property (nonatomic, strong) NSString * Status;
@property (nonatomic, strong) NSString * TimeUnit;
@property (nonatomic, strong) NSNumber * EstimatedTimeValue;
@property (nonatomic, strong) NSArray * SpentTimeTracking;
@property (nonatomic, strong) NSString * TotalSpentTime;
@property (nonatomic, strong) NSArray * AssignableUsers;
@property (nonatomic, strong) NSArray * TaskFlowNewStatuses;
@property (nonatomic, strong) NSArray * TaskFlowOpenStatuses;
@property (nonatomic, strong) NSArray * TaskFlowClosedStatuses;
@property (nonatomic, strong) NSArray * TaskLabels;
@property (nonatomic, strong) NSArray * TaskLogs;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- SharingInvitation ---
@interface QXSharingInvitation : NSObject

@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) NSString * FromUserId;
@property (nonatomic, strong) QXGroup * SharedGroup;
@property (nonatomic, assign) BOOL IsNewAccount;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSArray * JoinedOrgs;
@property (nonatomic, assign) BOOL IsAccepted;
@property (nonatomic, assign) BOOL IsRejected;
@property (nonatomic, assign) BOOL IsPending;
@property (nonatomic, assign) BOOL IsForwarded;
@property (nonatomic, assign) BOOL IsCanceled;
@property (nonatomic, assign) BOOL IsStopped;
@property (nonatomic, strong) NSString * PendingDuration;
@property (nonatomic, strong) NSString * ToOrgName;
@property (nonatomic, strong) NSString * ToOrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- User ---
@interface QXUser : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Firstame;
@property (nonatomic, strong) NSString * LastName;
@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Avatar;
@property (nonatomic, strong) NSString * JID;
@property (nonatomic, strong) NSString * Timezone;
@property (nonatomic, assign) BOOL IsSuperUser;
@property (nonatomic, assign) BOOL IsSharedUser;
@property (nonatomic, strong) NSString * OrgId;
@property (nonatomic, strong) NSString * OriginalOrgId;
@property (nonatomic, strong) NSString * PrefixURL;
@property (nonatomic, strong) NSString * ProfileURL;
@property (nonatomic, assign) BOOL IsLoggedInUser;
@property (nonatomic, assign) BOOL IsAvailable;
@property (nonatomic, assign) BOOL IsDisabled;
@property (nonatomic, assign) BOOL IsDeleted;
@property (nonatomic, assign) BOOL FromSharedGroup;
@property (nonatomic, strong) NSString * FromOrganizationName;
@property (nonatomic, assign) BOOL Editable;
@property (nonatomic, assign) BOOL Followable;
@property (nonatomic, assign) BOOL FollowedByMe;
@property (nonatomic, assign) BOOL FollowingTheGroup;
@property (nonatomic, strong) NSString * Department;
@property (nonatomic, strong) NSString * Location;
@property (nonatomic, strong) NSArray * FollowingGroups;
@property (nonatomic, strong) QXPreferences * Preferences;
@property (nonatomic, assign) BOOL NoDetail;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Task ---
@interface QXTask : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, assign) BOOL IsTaskOwner;
@property (nonatomic, assign) BOOL IsTaskAssignee;
@property (nonatomic, assign) BOOL IsOthers;
@property (nonatomic, assign) BOOL IsCurrentUserDone;
@property (nonatomic, assign) BOOL IsAcknowledgement;
@property (nonatomic, assign) BOOL IsTodoForOne;
@property (nonatomic, assign) BOOL IsTodoForAll;
@property (nonatomic, assign) BOOL IsCompleted;
@property (nonatomic, assign) BOOL IsClosed;
@property (nonatomic, assign) BOOL IsToGroup;
@property (nonatomic, assign) BOOL IsDueToday;
@property (nonatomic, assign) BOOL IsOverDue;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSDate * Due;
@property (nonatomic, strong) NSDate * CompletedAt;
@property (nonatomic, strong) NSString * LocalCreatedDate;
@property (nonatomic, strong) NSString * LocalDue;
@property (nonatomic, strong) NSString * LocalDueShortDate;
@property (nonatomic, strong) NSString * DueInputValue;
@property (nonatomic, strong) NSNumber * TotalUsersCount;
@property (nonatomic, strong) NSNumber * CompletedUsersCount;
@property (nonatomic, strong) NSNumber * PendingUsersCount;
@property (nonatomic, strong) QXEmbedUser * Owner;
@property (nonatomic, strong) NSArray * ToUsers;
@property (nonatomic, strong) NSString * ToUsersText;
@property (nonatomic, strong) NSArray * PendingUsers;
@property (nonatomic, strong) NSArray * CompletedUsers;
@property (nonatomic, strong) QXEmbedUser * Assignee;
@property (nonatomic, strong) NSString * ColorCssClass;
@property (nonatomic, strong) NSString * ReopenColorCssClass;
@property (nonatomic, strong) NSString * TaskBarHtml;
@property (nonatomic, strong) NSNumber * TaskFlow;
@property (nonatomic, assign) BOOL IsClaimed;
@property (nonatomic, assign) BOOL IsAdvancedTask;
@property (nonatomic, strong) QXAdvancedTask * AdvancedTask;
@property (nonatomic, assign) BOOL NeedShowAppliedText;
@property (nonatomic, assign) BOOL NeedToBeEditMode;
@property (nonatomic, assign) BOOL IsEditing;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- GroupAdvancedSettingPage ---
@interface QXGroupAdvancedSettingPage : NSObject

@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) NSArray * Followers;
@property (nonatomic, strong) QXOrganization * CurrentOrg;
@property (nonatomic, strong) QXGroupSharingInfo * SharingInfo;
@property (nonatomic, assign) BOOL CreatingGroup;
@property (nonatomic, assign) BOOL Editable;
@property (nonatomic, strong) NSDictionary * ThrowawayStatusSuggestions;
@property (nonatomic, strong) NSDictionary * ThrowawayNotYetOpenTagIndexes;
@property (nonatomic, strong) NSDictionary * ThrowawayOpenTagIndexes;
@property (nonatomic, strong) NSDictionary * ThrowawayClosedTagIndexes;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Member ---
@interface QXMember : NSObject

@property (nonatomic, strong) NSString * Name;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * ComfirmationSentAt;
@property (nonatomic, strong) NSString * SignupConfirmedAt;
@property (nonatomic, strong) NSString * SignupStatus;
@property (nonatomic, strong) NSArray * JoinedOrgs;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- OrgStats ---
@interface QXOrgStats : NSObject

@property (nonatomic, strong) QXOrganization * Organization;
@property (nonatomic, strong) NSNumber * UserCount;
@property (nonatomic, strong) NSNumber * GroupCount;
@property (nonatomic, strong) NSNumber * SharedGroupCount;
@property (nonatomic, strong) NSNumber * EntryCount;
@property (nonatomic, strong) NSNumber * CommentCount;
@property (nonatomic, strong) NSNumber * ChatCount;
@property (nonatomic, strong) NSString * CreatedAt;
@property (nonatomic, strong) NSString * LastUpdate;
@property (nonatomic, strong) QXEmbedUser * Author;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- Entry ---
@interface QXEntry : NSObject

@property (nonatomic, strong) NSString * Id;
@property (nonatomic, strong) NSString * EType;
@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * Slug;
@property (nonatomic, strong) NSString * Content;
@property (nonatomic, strong) NSString * TypeTitle;
@property (nonatomic, strong) NSString * RootId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AuthorId;
@property (nonatomic, strong) NSDate * CreatedAt;
@property (nonatomic, strong) NSDate * UpdatedAt;
@property (nonatomic, strong) NSDate * BumpedUpAt;
@property (nonatomic, strong) NSDate * VersionAt;
@property (nonatomic, strong) NSString * BaseOnEntryId;
@property (nonatomic, strong) NSString * AllAttachmentsURL;
@property (nonatomic, strong) NSString * Permalink;
@property (nonatomic, strong) NSString * IconName;
@property (nonatomic, strong) NSString * LocalHumanCreatedAt;
@property (nonatomic, strong) NSString * LocalHumanUpdatedAt;
@property (nonatomic, strong) NSString * WholeLastUpdateAtAgo;
@property (nonatomic, strong) NSDate * WholeLastUpdateAt;
@property (nonatomic, strong) NSString * LastUpdateAtAgo;
@property (nonatomic, strong) NSString * WatchedAtAgo;
@property (nonatomic, strong) NSString * NextRemindAtLater;
@property (nonatomic, strong) NSString * MentionedUserIds;
@property (nonatomic, strong) NSString * DomainURL;
@property (nonatomic, strong) NSString * UpdatedAtUnixNano;
@property (nonatomic, strong) NSString * LastUpdateAt;
@property (nonatomic, strong) NSString * HtmlTitle;
@property (nonatomic, strong) NSString * HtmlContent;
@property (nonatomic, strong) NSString * HtmlContentPart;
@property (nonatomic, strong) NSString * TaskHtmlContentPart;
@property (nonatomic, strong) NSString * WatchlistHtml;
@property (nonatomic, strong) NSString * ToUsersHtml;
@property (nonatomic, strong) NSString * LikedByUsersHtml;
@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSString * BaseOnLink;
@property (nonatomic, strong) NSString * BaseOnLinkTitle;
@property (nonatomic, strong) NSString * PresentationLink;
@property (nonatomic, strong) NSString * UploadURL;
@property (nonatomic, assign) BOOL IsShared;
@property (nonatomic, assign) BOOL IsPublished;
@property (nonatomic, assign) BOOL PublishedToUsers;
@property (nonatomic, assign) BOOL IsCanPublish;
@property (nonatomic, assign) BOOL IsPreferMarkdown;
@property (nonatomic, assign) BOOL IsMuted;
@property (nonatomic, assign) BOOL IsReminding;
@property (nonatomic, assign) BOOL IsSmartReminding;
@property (nonatomic, assign) BOOL IsNoReminding;
@property (nonatomic, strong) QXInnerMessage * InnerMessage;
@property (nonatomic, assign) BOOL IsSystemMessage;
@property (nonatomic, assign) BOOL IsInnerMessage;
@property (nonatomic, assign) BOOL IsBroadcast;
@property (nonatomic, assign) BOOL IsFromSuperOrg;
@property (nonatomic, assign) BOOL IsFromSuperUser;
@property (nonatomic, assign) BOOL IsFeedback;
@property (nonatomic, strong) QXEmbedOrg * FromOrg;
@property (nonatomic, strong) NSArray * ToOrgs;
@property (nonatomic, strong) NSString * ToOrgsHtml;
@property (nonatomic, assign) BOOL IsHidePresentationTip;
@property (nonatomic, assign) BOOL IsKnowledgeBase;
@property (nonatomic, assign) BOOL IsPost;
@property (nonatomic, assign) BOOL IsComment;
@property (nonatomic, assign) BOOL IsTask;
@property (nonatomic, assign) BOOL IsChat;
@property (nonatomic, assign) BOOL IsTaskToDo;
@property (nonatomic, assign) BOOL IsTaskAck;
@property (nonatomic, assign) BOOL IsTaskLog;
@property (nonatomic, assign) BOOL IsInWatchList;
@property (nonatomic, strong) NSString * IsToGroup;
@property (nonatomic, assign) BOOL CanEdit;
@property (nonatomic, assign) BOOL CanReply;
@property (nonatomic, assign) BOOL LikedByMe;
@property (nonatomic, assign) BOOL HasInlineTask;
@property (nonatomic, assign) BOOL TaskIsCompleted;
@property (nonatomic, assign) BOOL IsRoot;
@property (nonatomic, assign) BOOL IsUnread;
@property (nonatomic, assign) BOOL IsUpdated;
@property (nonatomic, assign) BOOL IsLastVersion;
@property (nonatomic, assign) BOOL Presentation;
@property (nonatomic, assign) BOOL AnyoneCanEdit;
@property (nonatomic, assign) BOOL IsInGroup;
@property (nonatomic, assign) BOOL IsFromEmail;
@property (nonatomic, assign) BOOL InlineHelp;
@property (nonatomic, strong) NSNumber * AllAttachmentsCount;
@property (nonatomic, strong) NSNumber * CommentsCount;
@property (nonatomic, strong) NSNumber * CurrentVersionCommentsCount;
@property (nonatomic, strong) NSNumber * AllLikesCount;
@property (nonatomic, strong) NSNumber * VersionCount;
@property (nonatomic, strong) NSNumber * UnreadCommentCount;
@property (nonatomic, strong) QXEmbedUser * Author;
@property (nonatomic, strong) QXEmbedUser * CurrentVersionEditor;
@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) QXTask * Todo;
@property (nonatomic, strong) QXTask * Ack;
@property (nonatomic, strong) QXConversation * Conversation;
@property (nonatomic, strong) NSArray * Versions;
@property (nonatomic, strong) NSArray * ToUsers;
@property (nonatomic, strong) NSArray * MentionedUsers;
@property (nonatomic, strong) NSArray * LikedByUsers;
@property (nonatomic, strong) NSArray * Attachments;
@property (nonatomic, strong) NSArray * CommentsAttachments;
@property (nonatomic, strong) QXAttachment * FirstPicture;
@property (nonatomic, strong) NSArray * ExternalComments;
@property (nonatomic, strong) NSArray * CurrentVersionComments;
@property (nonatomic, strong) NSArray * OtherVersionsComments;
@property (nonatomic, strong) QXEntry * NewComment;
@property (nonatomic, strong) QXEntry * NewEntry;
@property (nonatomic, strong) QXGroupSelector * GroupSlector;
@property (nonatomic, strong) NSDictionary * QortexSupportNotifyOptions;
@property (nonatomic, assign) BOOL IsQortexSupport;
@property (nonatomic, strong) QXQortexSupport * QortexSupport;
@property (nonatomic, assign) BOOL IsQortexSupportKnowledgeBase;
@property (nonatomic, strong) NSString * LinkTitle;
@property (nonatomic, assign) BOOL IsRequest;
@property (nonatomic, strong) QXShareRequest * ShareRequest;
@property (nonatomic, assign) BOOL VisibleForSuperUserInSuperOrg;
@property (nonatomic, assign) BOOL VisibleForSuperOrg;
@property (nonatomic, strong) NSString * CurrentLocaleName;
@property (nonatomic, strong) NSDictionary * LocaleTitleMap;
@property (nonatomic, strong) NSDictionary * LocaleContentMap;
@property (nonatomic, strong) NSArray * DerivedToDoEntries;
@property (nonatomic, strong) NSArray * RelatedToDoEntries;
@property (nonatomic, strong) QXBasedOnPost * BasedOnPost;
@property (nonatomic, strong) NSArray * RelatedEntries;
@property (nonatomic, strong) NSArray * LinkedEntries;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- MyChats ---
@interface QXMyChats : NSObject

@property (nonatomic, strong) NSArray * ChatEntries;
@property (nonatomic, assign) BOOL HasMore;
@property (nonatomic, strong) NSNumber * LatestCreateTime;
@property (nonatomic, assign) BOOL WhatChats;
@property (nonatomic, strong) NSString * PrefixURL;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- WatchItem ---
@interface QXWatchItem : NSObject

@property (nonatomic, strong) NSNumber * AttachCnt;
@property (nonatomic, strong) NSNumber * CommentCnt;
@property (nonatomic, strong) NSNumber * LikeCnt;
@property (nonatomic, strong) NSString * AttachCntStr;
@property (nonatomic, strong) NSString * CommentCntStr;
@property (nonatomic, strong) NSString * LikeCntStr;
@property (nonatomic, strong) NSDate * WatchTime;
@property (nonatomic, assign) BOOL IsSmartReminding;
@property (nonatomic, assign) BOOL IsNoReminding;
@property (nonatomic, strong) QXEntry * WatchEntry;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- DraftList ---
@interface QXDraftList : NSObject

@property (nonatomic, strong) NSArray * DraftItems;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- WatchList ---
@interface QXWatchList : NSObject

@property (nonatomic, strong) NSArray * Items;
@property (nonatomic, assign) BOOL WhatWatchList;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end


// === Interfaces ===

// --- QXGetNewOrganizationParams ---
@interface QXAuthMemberServiceGetNewOrganizationParams : NSObject

@property (nonatomic, strong) NSString * MemberId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewOrganizationResults ---
@interface QXAuthMemberServiceGetNewOrganizationResults : NSObject

@property (nonatomic, strong) QXOrganization * Org;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyOrganizationsParams ---
@interface QXAuthMemberServiceGetMyOrganizationsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyOrganizationsResults ---
@interface QXAuthMemberServiceGetMyOrganizationsResults : NSObject

@property (nonatomic, strong) NSArray * Orgs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateOrganizationParams ---
@interface QXAuthMemberServiceCreateOrganizationParams : NSObject

@property (nonatomic, strong) QXOrganizationInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateOrganizationResults ---
@interface QXAuthMemberServiceCreateOrganizationResults : NSObject

@property (nonatomic, strong) QXOrganization * ApiOrg;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXJoinOrganizationParams ---
@interface QXAuthMemberServiceJoinOrganizationParams : NSObject

@property (nonatomic, strong) NSString * OrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXJoinOrganizationResults ---
@interface QXAuthMemberServiceJoinOrganizationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXLeaveOrganizationParams ---
@interface QXAuthMemberServiceLeaveOrganizationParams : NSObject

@property (nonatomic, strong) NSString * OrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXLeaveOrganizationResults ---
@interface QXAuthMemberServiceLeaveOrganizationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSwitchOrganizationParams ---
@interface QXAuthMemberServiceSwitchOrganizationParams : NSObject

@property (nonatomic, strong) NSString * OrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSwitchOrganizationResults ---
@interface QXAuthMemberServiceSwitchOrganizationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAbandonInfoParams ---
@interface QXAuthMemberServiceGetAbandonInfoParams : NSObject

@property (nonatomic, strong) NSString * AbandonOrgId;
@property (nonatomic, strong) NSString * MemberId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAbandonInfoResults ---
@interface QXAuthMemberServiceGetAbandonInfoResults : NSObject

@property (nonatomic, strong) QXAbandonInfo * Info;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestParams ---
@interface QXAuthMemberServiceGetShareRequestParams : NSObject

@property (nonatomic, strong) NSString * Token;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestResults ---
@interface QXAuthMemberServiceGetShareRequestResults : NSObject

@property (nonatomic, strong) QXShareRequest * ShareRequest;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRejectShareRequestByInviteeParams ---
@interface QXAuthMemberServiceRejectShareRequestByInviteeParams : NSObject

@property (nonatomic, strong) NSString * Token;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRejectShareRequestByInviteeResults ---
@interface QXAuthMemberServiceRejectShareRequestByInviteeResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAcceptShareRequestByInviteeParams ---
@interface QXAuthMemberServiceAcceptShareRequestByInviteeParams : NSObject

@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * ToOrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAcceptShareRequestByInviteeResults ---
@interface QXAuthMemberServiceAcceptShareRequestByInviteeResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end


@interface QXAuthMemberService : NSObject- (NSDictionary*) dictionary;

- (QXAuthMemberServiceGetNewOrganizationResults *) GetNewOrganization:(NSString *)memberId;
- (void) getNewOrganization:(NSString *)memberId success:(void (^)(QXAuthMemberServiceGetNewOrganizationResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthMemberServiceGetMyOrganizationsResults *) GetMyOrganizations;
- (void) getMyOrganizations:(void (^)(QXAuthMemberServiceGetMyOrganizationsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthMemberServiceCreateOrganizationResults *) CreateOrganization:(QXOrganizationInput *)input;
- (void) createOrganization:(QXOrganizationInput *)input success:(void (^)(QXAuthMemberServiceCreateOrganizationResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) JoinOrganization:(NSString *)orgId;
- (void) joinOrganization:(NSString *)orgId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) LeaveOrganization:(NSString *)orgId;
- (void) leaveOrganization:(NSString *)orgId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) SwitchOrganization:(NSString *)orgId;
- (void) switchOrganization:(NSString *)orgId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthMemberServiceGetAbandonInfoResults *) GetAbandonInfo:(NSString *)abandonOrgId memberId:(NSString *)memberId;
- (void) getAbandonInfo:(NSString *)abandonOrgId memberId:(NSString *)memberId success:(void (^)(QXAuthMemberServiceGetAbandonInfoResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthMemberServiceGetShareRequestResults *) GetShareRequest:(NSString *)token;
- (void) getShareRequest:(NSString *)token success:(void (^)(QXAuthMemberServiceGetShareRequestResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RejectShareRequestByInvitee:(NSString *)token;
- (void) rejectShareRequestByInvitee:(NSString *)token success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) AcceptShareRequestByInvitee:(NSString *)token toOrgId:(NSString *)toOrgId;
- (void) acceptShareRequestByInvitee:(NSString *)token toOrgId:(NSString *)toOrgId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;
@end


// --- QXGetTotalStatsParams ---
@interface QXAuthAdminServiceGetTotalStatsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetTotalStatsResults ---
@interface QXAuthAdminServiceGetTotalStatsResults : NSObject

@property (nonatomic, strong) QXTotalStats * TotalStat;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetWeeklyTotalStatsParams ---
@interface QXAuthAdminServiceGetWeeklyTotalStatsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetWeeklyTotalStatsResults ---
@interface QXAuthAdminServiceGetWeeklyTotalStatsResults : NSObject

@property (nonatomic, strong) QXTotalStats * TotalStat;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgStatsParams ---
@interface QXAuthAdminServiceGetOrgStatsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgStatsResults ---
@interface QXAuthAdminServiceGetOrgStatsResults : NSObject

@property (nonatomic, strong) NSArray * OrgStats;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAccessRequestsParams ---
@interface QXAuthAdminServiceGetAccessRequestsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAccessRequestsResults ---
@interface QXAuthAdminServiceGetAccessRequestsResults : NSObject

@property (nonatomic, strong) NSArray * AccessReqs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXApproveAccessParams ---
@interface QXAuthAdminServiceApproveAccessParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXApproveAccessResults ---
@interface QXAuthAdminServiceApproveAccessResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResendApprovedMailParams ---
@interface QXAuthAdminServiceResendApprovedMailParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResendApprovedMailResults ---
@interface QXAuthAdminServiceResendApprovedMailResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAllMembersParams ---
@interface QXAuthAdminServiceGetAllMembersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAllMembersResults ---
@interface QXAuthAdminServiceGetAllMembersResults : NSObject

@property (nonatomic, strong) NSArray * Members;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXIgnoreAccessParams ---
@interface QXAuthAdminServiceIgnoreAccessParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXIgnoreAccessResults ---
@interface QXAuthAdminServiceIgnoreAccessResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAutoApproveAccessParams ---
@interface QXAuthAdminServiceGetAutoApproveAccessParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAutoApproveAccessResults ---
@interface QXAuthAdminServiceGetAutoApproveAccessResults : NSObject

@property (nonatomic, assign) BOOL Enabled;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSetAutoApproveAccessParams ---
@interface QXAuthAdminServiceSetAutoApproveAccessParams : NSObject

@property (nonatomic, assign) BOOL Enable;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSetAutoApproveAccessResults ---
@interface QXAuthAdminServiceSetAutoApproveAccessResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMarketableUsersParams ---
@interface QXAuthAdminServiceGetMarketableUsersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMarketableUsersResults ---
@interface QXAuthAdminServiceGetMarketableUsersResults : NSObject

@property (nonatomic, strong) NSArray * MemberInfos;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetTotalOnlineUsersParams ---
@interface QXAuthAdminServiceGetTotalOnlineUsersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetTotalOnlineUsersResults ---
@interface QXAuthAdminServiceGetTotalOnlineUsersResults : NSObject

@property (nonatomic, strong) NSArray * EmbedUsers;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end


@interface QXAuthAdminService : NSObject

@property (nonatomic, strong) NSString * Session;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;



- (QXAuthAdminServiceGetTotalStatsResults *) GetTotalStats;
- (void) getTotalStats:(void (^)(QXAuthAdminServiceGetTotalStatsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetWeeklyTotalStatsResults *) GetWeeklyTotalStats;
- (void) getWeeklyTotalStats:(void (^)(QXAuthAdminServiceGetWeeklyTotalStatsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetOrgStatsResults *) GetOrgStats;
- (void) getOrgStats:(void (^)(QXAuthAdminServiceGetOrgStatsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetAccessRequestsResults *) GetAccessRequests;
- (void) getAccessRequests:(void (^)(QXAuthAdminServiceGetAccessRequestsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ApproveAccess:(NSString *)email;
- (void) approveAccess:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ResendApprovedMail:(NSString *)email;
- (void) resendApprovedMail:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetAllMembersResults *) GetAllMembers;
- (void) getAllMembers:(void (^)(QXAuthAdminServiceGetAllMembersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) IgnoreAccess:(NSString *)email;
- (void) ignoreAccess:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetAutoApproveAccessResults *) GetAutoApproveAccess;
- (void) getAutoApproveAccess:(void (^)(QXAuthAdminServiceGetAutoApproveAccessResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) SetAutoApproveAccess:(BOOL)enable;
- (void) setAutoApproveAccess:(BOOL)enable success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetMarketableUsersResults *) GetMarketableUsers;
- (void) getMarketableUsers:(void (^)(QXAuthAdminServiceGetMarketableUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminServiceGetTotalOnlineUsersResults *) GetTotalOnlineUsers;
- (void) getTotalOnlineUsers:(void (^)(QXAuthAdminServiceGetTotalOnlineUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;
@end


// --- QXGetNewEntryParams ---
@interface QXAuthUserServiceGetNewEntryParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewEntryResults ---
@interface QXAuthUserServiceGetNewEntryResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewChatEntryParams ---
@interface QXAuthUserServiceGetNewChatEntryParams : NSObject

@property (nonatomic, strong) NSString * ChatId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewChatEntryResults ---
@interface QXAuthUserServiceGetNewChatEntryResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportEntriesParams ---
@interface QXAuthUserServiceGetQortexSupportEntriesParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;
@property (nonatomic, assign) BOOL WithComments;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportEntriesResults ---
@interface QXAuthUserServiceGetQortexSupportEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateEntryParams ---
@interface QXAuthUserServiceCreateEntryParams : NSObject

@property (nonatomic, strong) QXEntryInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateEntryResults ---
@interface QXAuthUserServiceCreateEntryResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateTaskParams ---
@interface QXAuthUserServiceCreateTaskParams : NSObject

@property (nonatomic, strong) QXEntryInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateTaskResults ---
@interface QXAuthUserServiceCreateTaskResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCloseTaskParams ---
@interface QXAuthUserServiceCloseTaskParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * TaskId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCloseTaskResults ---
@interface QXAuthUserServiceCloseTaskResults : NSObject

@property (nonatomic, strong) QXTask * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateCommentParams ---
@interface QXAuthUserServiceCreateCommentParams : NSObject

@property (nonatomic, strong) QXEntryInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateCommentResults ---
@interface QXAuthUserServiceCreateCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetCommentParams ---
@interface QXAuthUserServiceGetCommentParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetCommentResults ---
@interface QXAuthUserServiceGetCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateCommentParams ---
@interface QXAuthUserServiceUpdateCommentParams : NSObject

@property (nonatomic, strong) QXEntryInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateCommentResults ---
@interface QXAuthUserServiceUpdateCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateEntryParams ---
@interface QXAuthUserServiceUpdateEntryParams : NSObject

@property (nonatomic, strong) QXEntryInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateEntryResults ---
@interface QXAuthUserServiceUpdateEntryResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetLatestUpdatedEntryIdByTitleParams ---
@interface QXAuthUserServiceGetLatestUpdatedEntryIdByTitleParams : NSObject

@property (nonatomic, strong) NSString * Title;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetLatestUpdatedEntryIdByTitleResults ---
@interface QXAuthUserServiceGetLatestUpdatedEntryIdByTitleResults : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetEntryParams ---
@interface QXAuthUserServiceGetEntryParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * UpdateAtUnixNanoForVersion;
@property (nonatomic, strong) NSString * HightlightKeywords;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetEntryResults ---
@interface QXAuthUserServiceGetEntryResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteEntryParams ---
@interface QXAuthUserServiceDeleteEntryParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * DType;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteEntryResults ---
@interface QXAuthUserServiceDeleteEntryResults : NSObject

@property (nonatomic, strong) NSString * DelType;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMuteEntryParams ---
@interface QXAuthUserServiceMuteEntryParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMuteEntryResults ---
@interface QXAuthUserServiceMuteEntryResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUndoMuteEntryParams ---
@interface QXAuthUserServiceUndoMuteEntryParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUndoMuteEntryResults ---
@interface QXAuthUserServiceUndoMuteEntryResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMachineTranslatableLangaugesParams ---
@interface QXAuthUserServiceGetMachineTranslatableLangaugesParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMachineTranslatableLangaugesResults ---
@interface QXAuthUserServiceGetMachineTranslatableLangaugesResults : NSObject

@property (nonatomic, strong) QXLanguageSelector * Options;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMachineTranslateParams ---
@interface QXAuthUserServiceMachineTranslateParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * Targetlang;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMachineTranslateResults ---
@interface QXAuthUserServiceMachineTranslateResults : NSObject

@property (nonatomic, strong) QXTranslatedThread * TranslatedThread;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXOriginalThreadParams ---
@interface QXAuthUserServiceOriginalThreadParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXOriginalThreadResults ---
@interface QXAuthUserServiceOriginalThreadResults : NSObject

@property (nonatomic, strong) QXTranslatedThread * TranslatedThread;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetEntryAttachmentsParams ---
@interface QXAuthUserServiceGetEntryAttachmentsParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetEntryAttachmentsResults ---
@interface QXAuthUserServiceGetEntryAttachmentsResults : NSObject

@property (nonatomic, strong) NSArray * Attachments;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDocViewSessionParams ---
@interface QXAuthUserServiceGetDocViewSessionParams : NSObject

@property (nonatomic, strong) NSString * Doi;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AttachmentId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDocViewSessionResults ---
@interface QXAuthUserServiceGetDocViewSessionResults : NSObject

@property (nonatomic, strong) NSString * SessionId;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteEntryAttachmentParams ---
@interface QXAuthUserServiceDeleteEntryAttachmentParams : NSObject

@property (nonatomic, strong) NSString * Doi;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AttachmentId;
@property (nonatomic, strong) NSString * OwnerId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteEntryAttachmentResults ---
@interface QXAuthUserServiceDeleteEntryAttachmentResults : NSObject

@property (nonatomic, strong) NSArray * Attachments;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOtherVersionsCommentsParams ---
@interface QXAuthUserServiceGetOtherVersionsCommentsParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * UpdateAtUnixNanoForVersion;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOtherVersionsCommentsResults ---
@interface QXAuthUserServiceGetOtherVersionsCommentsResults : NSObject

@property (nonatomic, strong) NSArray * Comments;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOtherVersionsTaskLogsParams ---
@interface QXAuthUserServiceGetOtherVersionsTaskLogsParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * UpdateAtUnixNanoForVersion;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOtherVersionsTaskLogsResults ---
@interface QXAuthUserServiceGetOtherVersionsTaskLogsResults : NSObject

@property (nonatomic, strong) NSArray * TaskLogs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupEntriesParams ---
@interface QXAuthUserServiceGetGroupEntriesParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * EntryType;
@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;
@property (nonatomic, assign) BOOL WithComments;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupEntriesResults ---
@interface QXAuthUserServiceGetGroupEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyFeedEntriesParams ---
@interface QXAuthUserServiceGetMyFeedEntriesParams : NSObject

@property (nonatomic, strong) NSString * EntryType;
@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;
@property (nonatomic, assign) BOOL WithComments;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyFeedEntriesResults ---
@interface QXAuthUserServiceGetMyFeedEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupAsideParams ---
@interface QXAuthUserServiceGetGroupAsideParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupAsideResults ---
@interface QXAuthUserServiceGetGroupAsideResults : NSObject

@property (nonatomic, strong) QXGroupAside * Ga;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewFeedEntriesParams ---
@interface QXAuthUserServiceGetNewFeedEntriesParams : NSObject

@property (nonatomic, strong) NSString * EntryType;
@property (nonatomic, strong) NSString * FromTimeUnixNano;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewFeedEntriesResults ---
@interface QXAuthUserServiceGetNewFeedEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserEntriesParams ---
@interface QXAuthUserServiceGetUserEntriesParams : NSObject

@property (nonatomic, strong) NSString * UserId;
@property (nonatomic, strong) NSString * EntryType;
@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserEntriesResults ---
@interface QXAuthUserServiceGetUserEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyNotificationItemsParams ---
@interface QXAuthUserServiceGetMyNotificationItemsParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyNotificationItemsResults ---
@interface QXAuthUserServiceGetMyNotificationItemsResults : NSObject

@property (nonatomic, strong) NSArray * NotificationItems;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMarkAllAsReadParams ---
@interface QXAuthUserServiceMarkAllAsReadParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMarkAllAsReadResults ---
@interface QXAuthUserServiceMarkAllAsReadResults : NSObject

@property (nonatomic, strong) QXMyCount * Mycount;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetWatchListParams ---
@interface QXAuthUserServiceGetWatchListParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetWatchListResults ---
@interface QXAuthUserServiceGetWatchListResults : NSObject

@property (nonatomic, strong) QXWatchList * Watchlist;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAddToWatchListParams ---
@interface QXAuthUserServiceAddToWatchListParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * RemindMode;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAddToWatchListResults ---
@interface QXAuthUserServiceAddToWatchListResults : NSObject

@property (nonatomic, assign) BOOL Added;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopWatchingParams ---
@interface QXAuthUserServiceStopWatchingParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopWatchingResults ---
@interface QXAuthUserServiceStopWatchingResults : NSObject

@property (nonatomic, assign) BOOL Stopped;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXReadWatchingParams ---
@interface QXAuthUserServiceReadWatchingParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXReadWatchingResults ---
@interface QXAuthUserServiceReadWatchingResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRemindMeParams ---
@interface QXAuthUserServiceRemindMeParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRemindMeResults ---
@interface QXAuthUserServiceRemindMeResults : NSObject

@property (nonatomic, assign) BOOL Reminded;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStartSmartRemindingParams ---
@interface QXAuthUserServiceStartSmartRemindingParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * WatchItemId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStartSmartRemindingResults ---
@interface QXAuthUserServiceStartSmartRemindingResults : NSObject

@property (nonatomic, assign) BOOL Stopped;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopRemindingParams ---
@interface QXAuthUserServiceStopRemindingParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * WatchItemId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopRemindingResults ---
@interface QXAuthUserServiceStopRemindingResults : NSObject

@property (nonatomic, assign) BOOL Stopped;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateLikeParams ---
@interface QXAuthUserServiceUpdateLikeParams : NSObject

@property (nonatomic, strong) QXLikeInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateLikeResults ---
@interface QXAuthUserServiceUpdateLikeResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDraftListParams ---
@interface QXAuthUserServiceGetDraftListParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDraftListResults ---
@interface QXAuthUserServiceGetDraftListResults : NSObject

@property (nonatomic, strong) QXDraftList * Draftlist;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDraftParams ---
@interface QXAuthUserServiceGetDraftParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetDraftResults ---
@interface QXAuthUserServiceGetDraftResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteDraftParams ---
@interface QXAuthUserServiceDeleteDraftParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteDraftResults ---
@interface QXAuthUserServiceDeleteDraftResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChooseMarkdownEditorParams ---
@interface QXAuthUserServiceChooseMarkdownEditorParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChooseMarkdownEditorResults ---
@interface QXAuthUserServiceChooseMarkdownEditorResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChooseStyledEditorParams ---
@interface QXAuthUserServiceChooseStyledEditorParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChooseStyledEditorResults ---
@interface QXAuthUserServiceChooseStyledEditorResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewGroupParams ---
@interface QXAuthUserServiceGetNewGroupParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNewGroupResults ---
@interface QXAuthUserServiceGetNewGroupResults : NSObject

@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupParams ---
@interface QXAuthUserServiceGetGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupResults ---
@interface QXAuthUserServiceGetGroupResults : NSObject

@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateGroupParams ---
@interface QXAuthUserServiceCreateGroupParams : NSObject

@property (nonatomic, strong) QXGroupInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateGroupResults ---
@interface QXAuthUserServiceCreateGroupResults : NSObject

@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupParams ---
@interface QXAuthUserServiceUpdateGroupParams : NSObject

@property (nonatomic, strong) QXGroupInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupResults ---
@interface QXAuthUserServiceUpdateGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupLogoParams ---
@interface QXAuthUserServiceUpdateGroupLogoParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * LogoURL;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupLogoResults ---
@interface QXAuthUserServiceUpdateGroupLogoResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteGroupParams ---
@interface QXAuthUserServiceDeleteGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteGroupResults ---
@interface QXAuthUserServiceDeleteGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupBySlugParams ---
@interface QXAuthUserServiceGetGroupBySlugParams : NSObject

@property (nonatomic, strong) NSString * Slug;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupBySlugResults ---
@interface QXAuthUserServiceGetGroupBySlugResults : NSObject

@property (nonatomic, strong) QXGroup * Group;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupsParams ---
@interface QXAuthUserServiceGetGroupsParams : NSObject

@property (nonatomic, strong) NSString * Keyword;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupsResults ---
@interface QXAuthUserServiceGetGroupsResults : NSObject

@property (nonatomic, strong) NSArray * Groups;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPublicGroupsParams ---
@interface QXAuthUserServiceGetPublicGroupsParams : NSObject

@property (nonatomic, strong) NSString * Keyword;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPublicGroupsResults ---
@interface QXAuthUserServiceGetPublicGroupsResults : NSObject

@property (nonatomic, strong) NSArray * Groups;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAddUserToGroupParams ---
@interface QXAuthUserServiceAddUserToGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAddUserToGroupResults ---
@interface QXAuthUserServiceAddUserToGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRemoveUserFromGroupParams ---
@interface QXAuthUserServiceRemoveUserFromGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRemoveUserFromGroupResults ---
@interface QXAuthUserServiceRemoveUserFromGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClassifiedGroupsParams ---
@interface QXAuthUserServiceGetClassifiedGroupsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClassifiedGroupsResults ---
@interface QXAuthUserServiceGetClassifiedGroupsResults : NSObject

@property (nonatomic, strong) QXGroup * AnouncementGroup;
@property (nonatomic, strong) QXGroup * SmGroup;
@property (nonatomic, strong) NSArray * FollowedNormalGroups;
@property (nonatomic, strong) NSArray * FollowedSharedGroups;
@property (nonatomic, strong) NSArray * UnFollowedNormalGroups;
@property (nonatomic, strong) NSArray * UnFollowedSharedGroups;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXBulkUpdateTasksInGroupParams ---
@interface QXAuthUserServiceBulkUpdateTasksInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSArray * TaskPwMap;
@property (nonatomic, strong) NSArray * TaskInputs;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXBulkUpdateTasksInGroupResults ---
@interface QXAuthUserServiceBulkUpdateTasksInGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAuthUserParams ---
@interface QXAuthUserServiceGetAuthUserParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAuthUserResults ---
@interface QXAuthUserServiceGetAuthUserResults : NSObject

@property (nonatomic, strong) QXUser * User;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgUsersParams ---
@interface QXAuthUserServiceGetOrgUsersParams : NSObject

@property (nonatomic, strong) NSString * Keyword;
@property (nonatomic, strong) NSString * StartFullName;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgUsersResults ---
@interface QXAuthUserServiceGetOrgUsersResults : NSObject

@property (nonatomic, strong) NSArray * Users;
@property (nonatomic, strong) NSString * NextFullName;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupUsersParams ---
@interface QXAuthUserServiceGetGroupUsersParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * Keyword;
@property (nonatomic, assign) BOOL OnlyFollowers;
@property (nonatomic, strong) NSString * StartFullName;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupUsersResults ---
@interface QXAuthUserServiceGetGroupUsersResults : NSObject

@property (nonatomic, strong) NSArray * Users;
@property (nonatomic, strong) NSString * NextFullName;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserParams ---
@interface QXAuthUserServiceGetUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserResults ---
@interface QXAuthUserServiceGetUserResults : NSObject

@property (nonatomic, strong) QXUser * User;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXEnableUserParams ---
@interface QXAuthUserServiceEnableUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXEnableUserResults ---
@interface QXAuthUserServiceEnableUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDisableUserParams ---
@interface QXAuthUserServiceDisableUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDisableUserResults ---
@interface QXAuthUserServiceDisableUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteUserParams ---
@interface QXAuthUserServiceDeleteUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDeleteUserResults ---
@interface QXAuthUserServiceDeleteUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPromoteToSuperUserParams ---
@interface QXAuthUserServicePromoteToSuperUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPromoteToSuperUserResults ---
@interface QXAuthUserServicePromoteToSuperUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDemoteFromSuperUserParams ---
@interface QXAuthUserServiceDemoteFromSuperUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDemoteFromSuperUserResults ---
@interface QXAuthUserServiceDemoteFromSuperUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXFollowUserParams ---
@interface QXAuthUserServiceFollowUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXFollowUserResults ---
@interface QXAuthUserServiceFollowUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUnfollowUserParams ---
@interface QXAuthUserServiceUnfollowUserParams : NSObject

@property (nonatomic, strong) NSString * UserId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUnfollowUserResults ---
@interface QXAuthUserServiceUnfollowUserResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyFollowingUsersParams ---
@interface QXAuthUserServiceGetMyFollowingUsersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyFollowingUsersResults ---
@interface QXAuthUserServiceGetMyFollowingUsersResults : NSObject

@property (nonatomic, strong) NSArray * FollowingUsers;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPanelStatusParams ---
@interface QXAuthUserServiceGetPanelStatusParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPanelStatusResults ---
@interface QXAuthUserServiceGetPanelStatusResults : NSObject

@property (nonatomic, strong) QXPanelStatus * PanelStatus;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserPreferencesParams ---
@interface QXAuthUserServiceGetUserPreferencesParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetUserPreferencesResults ---
@interface QXAuthUserServiceGetUserPreferencesResults : NSObject

@property (nonatomic, strong) QXPreferences * Preferences;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateUserPreferencesParams ---
@interface QXAuthUserServiceUpdateUserPreferencesParams : NSObject

@property (nonatomic, strong) QXPreferencesInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateUserPreferencesResults ---
@interface QXAuthUserServiceUpdateUserPreferencesResults : NSObject

@property (nonatomic, strong) QXPreferences * Preferences;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgEmbedUsersParams ---
@interface QXAuthUserServiceGetOrgEmbedUsersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgEmbedUsersResults ---
@interface QXAuthUserServiceGetOrgEmbedUsersResults : NSObject

@property (nonatomic, strong) NSArray * Users;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNonStandardGroupEmbedUsersParams ---
@interface QXAuthUserServiceGetNonStandardGroupEmbedUsersParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetNonStandardGroupEmbedUsersResults ---
@interface QXAuthUserServiceGetNonStandardGroupEmbedUsersResults : NSObject

@property (nonatomic, strong) NSArray * GroupUsers;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateUserProfileParams ---
@interface QXAuthUserServiceUpdateUserProfileParams : NSObject

@property (nonatomic, strong) QXUserProfileInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateUserProfileResults ---
@interface QXAuthUserServiceUpdateUserProfileResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSetPreferredLanguagesParams ---
@interface QXAuthUserServiceSetPreferredLanguagesParams : NSObject

@property (nonatomic, strong) NSArray * LanguageCodes;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSetPreferredLanguagesResults ---
@interface QXAuthUserServiceSetPreferredLanguagesResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyCountParams ---
@interface QXAuthUserServiceGetMyCountParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyCountResults ---
@interface QXAuthUserServiceGetMyCountResults : NSObject

@property (nonatomic, strong) QXMyCount * MyCount;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXReadEntryParams ---
@interface QXAuthUserServiceReadEntryParams : NSObject

@property (nonatomic, strong) NSString * EntryId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXReadEntryResults ---
@interface QXAuthUserServiceReadEntryResults : NSObject

@property (nonatomic, strong) QXMyCount * MyCount;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetJoinOrgInvitationsParams ---
@interface QXAuthUserServiceGetJoinOrgInvitationsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetJoinOrgInvitationsResults ---
@interface QXAuthUserServiceGetJoinOrgInvitationsResults : NSObject

@property (nonatomic, strong) NSArray * Invitations;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrganizationParams ---
@interface QXAuthUserServiceGetOrganizationParams : NSObject

@property (nonatomic, strong) NSString * OrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrganizationResults ---
@interface QXAuthUserServiceGetOrganizationResults : NSObject

@property (nonatomic, strong) QXOrganization * Org;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrganizationsParams ---
@interface QXAuthUserServiceGetOrganizationsParams : NSObject

@property (nonatomic, strong) NSArray * OrgIds;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrganizationsResults ---
@interface QXAuthUserServiceGetOrganizationsResults : NSObject

@property (nonatomic, strong) NSArray * Orgs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyOrgsUnreadInfoParams ---
@interface QXAuthUserServiceGetMyOrgsUnreadInfoParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyOrgsUnreadInfoResults ---
@interface QXAuthUserServiceGetMyOrgsUnreadInfoResults : NSObject

@property (nonatomic, strong) NSArray * UnreadInfo;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyJoinedOrganizationsParams ---
@interface QXAuthUserServiceGetMyJoinedOrganizationsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyJoinedOrganizationsResults ---
@interface QXAuthUserServiceGetMyJoinedOrganizationsResults : NSObject

@property (nonatomic, strong) NSArray * Orgs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetCurrentOrganizationParams ---
@interface QXAuthUserServiceGetCurrentOrganizationParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetCurrentOrganizationResults ---
@interface QXAuthUserServiceGetCurrentOrganizationResults : NSObject

@property (nonatomic, strong) QXOrganization * Org;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSearchOrganizationsParams ---
@interface QXAuthUserServiceSearchOrganizationsParams : NSObject

@property (nonatomic, strong) NSString * Keyword;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSearchOrganizationsResults ---
@interface QXAuthUserServiceSearchOrganizationsResults : NSObject

@property (nonatomic, strong) NSArray * Orgs;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateOrganizationParams ---
@interface QXAuthUserServiceUpdateOrganizationParams : NSObject

@property (nonatomic, strong) QXOrganizationInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateOrganizationResults ---
@interface QXAuthUserServiceUpdateOrganizationResults : NSObject

@property (nonatomic, strong) QXOrganization * Org;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSwitchOrganizationParams ---
@interface QXAuthUserServiceSwitchOrganizationParams : NSObject

@property (nonatomic, strong) NSString * OrgId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSwitchOrganizationResults ---
@interface QXAuthUserServiceSwitchOrganizationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAcceptShareRequestByAdminParams ---
@interface QXAuthUserServiceAcceptShareRequestByAdminParams : NSObject

@property (nonatomic, strong) NSString * RequestId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAcceptShareRequestByAdminResults ---
@interface QXAuthUserServiceAcceptShareRequestByAdminResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRejectShareRequestByAdminParams ---
@interface QXAuthUserServiceRejectShareRequestByAdminParams : NSObject

@property (nonatomic, strong) NSString * RequestId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRejectShareRequestByAdminResults ---
@interface QXAuthUserServiceRejectShareRequestByAdminResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStartTrialParams ---
@interface QXAuthUserServiceStartTrialParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStartTrialResults ---
@interface QXAuthUserServiceStartTrialResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgSettingsParams ---
@interface QXAuthUserServiceGetOrgSettingsParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOrgSettingsResults ---
@interface QXAuthUserServiceGetOrgSettingsResults : NSObject

@property (nonatomic, strong) QXOrgSettings * OrgSetting;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateOrgSettingsParams ---
@interface QXAuthUserServiceUpdateOrgSettingsParams : NSObject

@property (nonatomic, strong) QXOrgSettingsInput * OrgSettingInput;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateOrgSettingsResults ---
@interface QXAuthUserServiceUpdateOrgSettingsResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCanCreateGroupParams ---
@interface QXAuthUserServiceCanCreateGroupParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCanCreateGroupResults ---
@interface QXAuthUserServiceCanCreateGroupResults : NSObject

@property (nonatomic, assign) BOOL Ok;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCanInvitePeopleParams ---
@interface QXAuthUserServiceCanInvitePeopleParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCanInvitePeopleResults ---
@interface QXAuthUserServiceCanInvitePeopleResults : NSObject

@property (nonatomic, assign) BOOL Ok;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXInvitePeopleParams ---
@interface QXAuthUserServiceInvitePeopleParams : NSObject

@property (nonatomic, strong) NSArray * Emails;
@property (nonatomic, assign) BOOL AllowEmpty;
@property (nonatomic, assign) BOOL SkipInvalidEmail;
@property (nonatomic, strong) NSString * CustomMessage;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXInvitePeopleResults ---
@interface QXAuthUserServiceInvitePeopleResults : NSObject

@property (nonatomic, strong) NSArray * SendedEmails;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelInvitationParams ---
@interface QXAuthUserServiceCancelInvitationParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelInvitationResults ---
@interface QXAuthUserServiceCancelInvitationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResendInvitationParams ---
@interface QXAuthUserServiceResendInvitationParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResendInvitationResults ---
@interface QXAuthUserServiceResendInvitationResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChangeLocaleParams ---
@interface QXAuthUserServiceChangeLocaleParams : NSObject

@property (nonatomic, strong) NSString * LocaleName;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChangeLocaleResults ---
@interface QXAuthUserServiceChangeLocaleResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupAdvancedToDoSettingsParams ---
@interface QXAuthUserServiceUpdateGroupAdvancedToDoSettingsParams : NSObject

@property (nonatomic, strong) NSString * GId;
@property (nonatomic, strong) NSString * Settings;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateGroupAdvancedToDoSettingsResults ---
@interface QXAuthUserServiceUpdateGroupAdvancedToDoSettingsResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateMailPreferenceParams ---
@interface QXAuthUserServiceUpdateMailPreferenceParams : NSObject

@property (nonatomic, strong) QXMailPreferenceInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateMailPreferenceResults ---
@interface QXAuthUserServiceUpdateMailPreferenceResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPrepareChangingEmailParams ---
@interface QXAuthUserServicePrepareChangingEmailParams : NSObject

@property (nonatomic, strong) NSString * NewEmail;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPrepareChangingEmailResults ---
@interface QXAuthUserServicePrepareChangingEmailResults : NSObject

@property (nonatomic, strong) QXEmailChanger * Changer;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXConfirmChangingEmailParams ---
@interface QXAuthUserServiceConfirmChangingEmailParams : NSObject

@property (nonatomic, strong) NSString * Token;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXConfirmChangingEmailResults ---
@interface QXAuthUserServiceConfirmChangingEmailResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateAccountParams ---
@interface QXAuthUserServiceUpdateAccountParams : NSObject

@property (nonatomic, strong) QXMemberAccountInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateAccountResults ---
@interface QXAuthUserServiceUpdateAccountResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSendShareRequestParams ---
@interface QXAuthUserServiceSendShareRequestParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXSendShareRequestResults ---
@interface QXAuthUserServiceSendShareRequestResults : NSObject

@property (nonatomic, strong) QXShareRequest * ShareRequest;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestsParams ---
@interface QXAuthUserServiceGetShareRequestsParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestsResults ---
@interface QXAuthUserServiceGetShareRequestsResults : NSObject

@property (nonatomic, strong) NSArray * Sis;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelShareRequestParams ---
@interface QXAuthUserServiceCancelShareRequestParams : NSObject

@property (nonatomic, strong) NSString * RequestId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelShareRequestResults ---
@interface QXAuthUserServiceCancelShareRequestResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopSharingGroupParams ---
@interface QXAuthUserServiceStopSharingGroupParams : NSObject

@property (nonatomic, strong) NSString * RequestId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXStopSharingGroupResults ---
@interface QXAuthUserServiceStopSharingGroupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDismissPresentationTipParams ---
@interface QXAuthUserServiceDismissPresentationTipParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXDismissPresentationTipResults ---
@interface QXAuthUserServiceDismissPresentationTipResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyChatEntriesParams ---
@interface QXAuthUserServiceGetMyChatEntriesParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetMyChatEntriesResults ---
@interface QXAuthUserServiceGetMyChatEntriesResults : NSObject

@property (nonatomic, strong) NSArray * Entries;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPrivateChatParams ---
@interface QXAuthUserServiceGetPrivateChatParams : NSObject

@property (nonatomic, strong) NSString * ConversationId;
@property (nonatomic, strong) NSString * SearchKeyWords;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetPrivateChatResults ---
@interface QXAuthUserServiceGetPrivateChatResults : NSObject

@property (nonatomic, strong) QXEntry * ChatEntry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateQortexSupportParams ---
@interface QXAuthUserServiceCreateQortexSupportParams : NSObject

@property (nonatomic, strong) QXQortexSupportInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateQortexSupportResults ---
@interface QXAuthUserServiceCreateQortexSupportResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateQortexSupportCommentParams ---
@interface QXAuthUserServiceCreateQortexSupportCommentParams : NSObject

@property (nonatomic, strong) QXQortexSupportInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateQortexSupportCommentResults ---
@interface QXAuthUserServiceCreateQortexSupportCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportParams ---
@interface QXAuthUserServiceGetQortexSupportParams : NSObject

@property (nonatomic, strong) NSString * EntryId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportResults ---
@interface QXAuthUserServiceGetQortexSupportResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportCommentParams ---
@interface QXAuthUserServiceGetQortexSupportCommentParams : NSObject

@property (nonatomic, strong) NSString * EntryId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportCommentResults ---
@interface QXAuthUserServiceGetQortexSupportCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateQortexSupportParams ---
@interface QXAuthUserServiceUpdateQortexSupportParams : NSObject

@property (nonatomic, strong) QXQortexSupportInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateQortexSupportResults ---
@interface QXAuthUserServiceUpdateQortexSupportResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateQortexSupportCommentParams ---
@interface QXAuthUserServiceUpdateQortexSupportCommentParams : NSObject

@property (nonatomic, strong) QXQortexSupportInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateQortexSupportCommentResults ---
@interface QXAuthUserServiceUpdateQortexSupportCommentResults : NSObject

@property (nonatomic, strong) QXEntry * Entry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportHelpLinkParams ---
@interface QXAuthUserServiceGetQortexSupportHelpLinkParams : NSObject

@property (nonatomic, strong) NSString * Title;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetQortexSupportHelpLinkResults ---
@interface QXAuthUserServiceGetQortexSupportHelpLinkResults : NSObject

@property (nonatomic, strong) NSString * Link;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXNewTaskParams ---
@interface QXAuthUserServiceNewTaskParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXNewTaskResults ---
@interface QXAuthUserServiceNewTaskResults : NSObject

@property (nonatomic, strong) QXTask * Task;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXEditTaskParams ---
@interface QXAuthUserServiceEditTaskParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * TaskId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXEditTaskResults ---
@interface QXAuthUserServiceEditTaskResults : NSObject

@property (nonatomic, strong) QXTask * Task;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAdvancedTaskParams ---
@interface QXAuthUserServiceGetAdvancedTaskParams : NSObject

@property (nonatomic, strong) NSString * TaskId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAdvancedTaskResults ---
@interface QXAuthUserServiceGetAdvancedTaskResults : NSObject

@property (nonatomic, strong) QXAdvancedTask * At;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXClaimTaskParams ---
@interface QXAuthUserServiceClaimTaskParams : NSObject

@property (nonatomic, strong) NSString * TaskId;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXClaimTaskResults ---
@interface QXAuthUserServiceClaimTaskResults : NSObject

@property (nonatomic, strong) QXTask * Task;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateTaskParams ---
@interface QXAuthUserServiceUpdateTaskParams : NSObject

@property (nonatomic, strong) QXTaskInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXUpdateTaskResults ---
@interface QXAuthUserServiceUpdateTaskResults : NSObject

@property (nonatomic, strong) QXTask * Task;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetTasksForMeParams ---
@interface QXAuthUserServiceGetTasksForMeParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetTasksForMeResults ---
@interface QXAuthUserServiceGetTasksForMeResults : NSObject

@property (nonatomic, strong) NSArray * NeedActionTasks;
@property (nonatomic, strong) NSArray * GroupTasks;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOpenTasksIMadeParams ---
@interface QXAuthUserServiceGetOpenTasksIMadeParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOpenTasksIMadeResults ---
@interface QXAuthUserServiceGetOpenTasksIMadeResults : NSObject

@property (nonatomic, strong) NSArray * GroupTasks;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClosedTasksIMadeParams ---
@interface QXAuthUserServiceGetClosedTasksIMadeParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClosedTasksIMadeResults ---
@interface QXAuthUserServiceGetClosedTasksIMadeResults : NSObject

@property (nonatomic, strong) NSArray * Tasks;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOpenTasksIWorkedOnParams ---
@interface QXAuthUserServiceGetOpenTasksIWorkedOnParams : NSObject


- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetOpenTasksIWorkedOnResults ---
@interface QXAuthUserServiceGetOpenTasksIWorkedOnResults : NSObject

@property (nonatomic, strong) NSArray * GroupTasks;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClosedTasksIWorkedOnParams ---
@interface QXAuthUserServiceGetClosedTasksIWorkedOnParams : NSObject

@property (nonatomic, strong) NSString * Before;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetClosedTasksIWorkedOnResults ---
@interface QXAuthUserServiceGetClosedTasksIWorkedOnResults : NSObject

@property (nonatomic, strong) NSArray * Tasks;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupAdvancedToDoSettingParams ---
@interface QXAuthUserServiceGetGroupAdvancedToDoSettingParams : NSObject

@property (nonatomic, strong) NSString * GId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetGroupAdvancedToDoSettingResults ---
@interface QXAuthUserServiceGetGroupAdvancedToDoSettingResults : NSObject

@property (nonatomic, strong) QXGroupAdvancedSettingPage * Page;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosInGroupParams ---
@interface QXAuthUserServiceAllOpenAdvancedToDosInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosInGroupResults ---
@interface QXAuthUserServiceAllOpenAdvancedToDosInGroupResults : NSObject

@property (nonatomic, strong) QXOpenAdvancedToDosPage * Page;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByUserInGroupParams ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByUserInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByUserInGroupResults ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByUserInGroupResults : NSObject

@property (nonatomic, strong) NSArray * Atos;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByStatusInGroupParams ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByStatusInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByStatusInGroupResults ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByStatusInGroupResults : NSObject

@property (nonatomic, strong) QXOpenAdvancedToDosPage * Page;
@property (nonatomic, strong) QXGroup * ApiGroup;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByLabelInGroupParams ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByLabelInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenAdvancedToDosGroupingByLabelInGroupResults ---
@interface QXAuthUserServiceAllOpenAdvancedToDosGroupingByLabelInGroupResults : NSObject

@property (nonatomic, strong) QXOpenAdvancedToDosPage * Page;
@property (nonatomic, strong) QXGroup * ApiGroup;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenBasicToDosInGroupParams ---
@interface QXAuthUserServiceAllOpenBasicToDosInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenBasicToDosInGroupResults ---
@interface QXAuthUserServiceAllOpenBasicToDosInGroupResults : NSObject

@property (nonatomic, strong) NSArray * TaskOutlines;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenBasicToDosGroupingByUserInGroupParams ---
@interface QXAuthUserServiceAllOpenBasicToDosGroupingByUserInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllOpenBasicToDosGroupingByUserInGroupResults ---
@interface QXAuthUserServiceAllOpenBasicToDosGroupingByUserInGroupResults : NSObject

@property (nonatomic, strong) NSArray * Atos;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllClosedBasicToDosInGroupParams ---
@interface QXAuthUserServiceAllClosedBasicToDosInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSString * AfterTimeS;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllClosedBasicToDosInGroupResults ---
@interface QXAuthUserServiceAllClosedBasicToDosInGroupResults : NSObject

@property (nonatomic, strong) NSArray * TaskOutlines;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllClosedAdvancedToDosInGroupParams ---
@interface QXAuthUserServiceAllClosedAdvancedToDosInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXAllClosedAdvancedToDosInGroupResults ---
@interface QXAuthUserServiceAllClosedAdvancedToDosInGroupResults : NSObject

@property (nonatomic, strong) NSArray * ClosedOutlines;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMoreClosedAdvancedToDosWithStatusInGroupParams ---
@interface QXAuthUserServiceMoreClosedAdvancedToDosWithStatusInGroupParams : NSObject

@property (nonatomic, strong) NSString * GroupId;
@property (nonatomic, strong) NSNumber * Status;
@property (nonatomic, strong) NSString * AfterTime;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXMoreClosedAdvancedToDosWithStatusInGroupResults ---
@interface QXAuthUserServiceMoreClosedAdvancedToDosWithStatusInGroupResults : NSObject

@property (nonatomic, strong) NSArray * TaskOutlines;
@property (nonatomic, strong) QXGroup * ApiGroup;
@property (nonatomic, assign) BOOL HasMore;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCountOfClosedToDosInGroupParams ---
@interface QXAuthUserServiceCountOfClosedToDosInGroupParams : NSObject

@property (nonatomic, strong) NSNumber * Ttype;
@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCountOfClosedToDosInGroupResults ---
@interface QXAuthUserServiceCountOfClosedToDosInGroupResults : NSObject

@property (nonatomic, strong) NSNumber * Count;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXToDoCSVParams ---
@interface QXAuthUserServiceToDoCSVParams : NSObject

@property (nonatomic, strong) NSString * GroupId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXToDoCSVResults ---
@interface QXAuthUserServiceToDoCSVResults : NSObject

@property (nonatomic, strong) NSArray * Todos;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end


@interface QXAuthUserService : NSObject

@property (nonatomic, strong) NSString * Session;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;



- (QXAuthUserServiceGetNewEntryResults *) GetNewEntry:(NSString *)groupId;
- (void) getNewEntry:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetNewEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetNewChatEntryResults *) GetNewChatEntry:(NSString *)chatId;
- (void) getNewChatEntry:(NSString *)chatId success:(void (^)(QXAuthUserServiceGetNewChatEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetQortexSupportEntriesResults *) GetQortexSupportEntries:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments;
- (void) getQortexSupportEntries:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments success:(void (^)(QXAuthUserServiceGetQortexSupportEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateEntryResults *) CreateEntry:(QXEntryInput *)input;
- (void) createEntry:(QXEntryInput *)input success:(void (^)(QXAuthUserServiceCreateEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateTaskResults *) CreateTask:(QXEntryInput *)input;
- (void) createTask:(QXEntryInput *)input success:(void (^)(QXAuthUserServiceCreateTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCloseTaskResults *) CloseTask:(NSString *)entryId groupId:(NSString *)groupId taskId:(NSString *)taskId;
- (void) closeTask:(NSString *)entryId groupId:(NSString *)groupId taskId:(NSString *)taskId success:(void (^)(QXAuthUserServiceCloseTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateCommentResults *) CreateComment:(QXEntryInput *)input;
- (void) createComment:(QXEntryInput *)input success:(void (^)(QXAuthUserServiceCreateCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetCommentResults *) GetComment:(NSString *)entryId groupId:(NSString *)groupId;
- (void) getComment:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateCommentResults *) UpdateComment:(QXEntryInput *)input;
- (void) updateComment:(QXEntryInput *)input success:(void (^)(QXAuthUserServiceUpdateCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateEntryResults *) UpdateEntry:(QXEntryInput *)input;
- (void) updateEntry:(QXEntryInput *)input success:(void (^)(QXAuthUserServiceUpdateEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetLatestUpdatedEntryIdByTitleResults *) GetLatestUpdatedEntryIdByTitle:(NSString *)title groupId:(NSString *)groupId;
- (void) getLatestUpdatedEntryIdByTitle:(NSString *)title groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetLatestUpdatedEntryIdByTitleResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetEntryResults *) GetEntry:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion hightlightKeywords:(NSString *)hightlightKeywords;
- (void) getEntry:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion hightlightKeywords:(NSString *)hightlightKeywords success:(void (^)(QXAuthUserServiceGetEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceDeleteEntryResults *) DeleteEntry:(NSString *)entryId groupId:(NSString *)groupId dType:(NSString *)dType;
- (void) deleteEntry:(NSString *)entryId groupId:(NSString *)groupId dType:(NSString *)dType success:(void (^)(QXAuthUserServiceDeleteEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) MuteEntry:(NSString *)entryId groupId:(NSString *)groupId;
- (void) muteEntry:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UndoMuteEntry:(NSString *)entryId groupId:(NSString *)groupId;
- (void) undoMuteEntry:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMachineTranslatableLangaugesResults *) GetMachineTranslatableLangauges;
- (void) getMachineTranslatableLangauges:(void (^)(QXAuthUserServiceGetMachineTranslatableLangaugesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceMachineTranslateResults *) MachineTranslate:(NSString *)entryId groupId:(NSString *)groupId targetlang:(NSString *)targetlang;
- (void) machineTranslate:(NSString *)entryId groupId:(NSString *)groupId targetlang:(NSString *)targetlang success:(void (^)(QXAuthUserServiceMachineTranslateResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceOriginalThreadResults *) OriginalThread:(NSString *)entryId groupId:(NSString *)groupId;
- (void) originalThread:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceOriginalThreadResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetEntryAttachmentsResults *) GetEntryAttachments:(NSString *)entryId groupId:(NSString *)groupId;
- (void) getEntryAttachments:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetEntryAttachmentsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetDocViewSessionResults *) GetDocViewSession:(NSString *)doi groupId:(NSString *)groupId attachmentId:(NSString *)attachmentId;
- (void) getDocViewSession:(NSString *)doi groupId:(NSString *)groupId attachmentId:(NSString *)attachmentId success:(void (^)(QXAuthUserServiceGetDocViewSessionResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceDeleteEntryAttachmentResults *) DeleteEntryAttachment:(NSString *)doi groupId:(NSString *)groupId attachmentId:(NSString *)attachmentId ownerId:(NSString *)ownerId;
- (void) deleteEntryAttachment:(NSString *)doi groupId:(NSString *)groupId attachmentId:(NSString *)attachmentId ownerId:(NSString *)ownerId success:(void (^)(QXAuthUserServiceDeleteEntryAttachmentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOtherVersionsCommentsResults *) GetOtherVersionsComments:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion;
- (void) getOtherVersionsComments:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion success:(void (^)(QXAuthUserServiceGetOtherVersionsCommentsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOtherVersionsTaskLogsResults *) GetOtherVersionsTaskLogs:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion;
- (void) getOtherVersionsTaskLogs:(NSString *)entryId groupId:(NSString *)groupId updateAtUnixNanoForVersion:(NSString *)updateAtUnixNanoForVersion success:(void (^)(QXAuthUserServiceGetOtherVersionsTaskLogsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupEntriesResults *) GetGroupEntries:(NSString *)groupId entryType:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments;
- (void) getGroupEntries:(NSString *)groupId entryType:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments success:(void (^)(QXAuthUserServiceGetGroupEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyFeedEntriesResults *) GetMyFeedEntries:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments;
- (void) getMyFeedEntries:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit withComments:(BOOL)withComments success:(void (^)(QXAuthUserServiceGetMyFeedEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupAsideResults *) GetGroupAside;
- (void) getGroupAside:(void (^)(QXAuthUserServiceGetGroupAsideResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetNewFeedEntriesResults *) GetNewFeedEntries:(NSString *)entryType fromTimeUnixNano:(NSString *)fromTimeUnixNano limit:(NSNumber *)limit;
- (void) getNewFeedEntries:(NSString *)entryType fromTimeUnixNano:(NSString *)fromTimeUnixNano limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetNewFeedEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetUserEntriesResults *) GetUserEntries:(NSString *)userId entryType:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit;
- (void) getUserEntries:(NSString *)userId entryType:(NSString *)entryType before:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetUserEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyNotificationItemsResults *) GetMyNotificationItems:(NSString *)before limit:(NSNumber *)limit;
- (void) getMyNotificationItems:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetMyNotificationItemsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceMarkAllAsReadResults *) MarkAllAsRead:(NSString *)groupId;
- (void) markAllAsRead:(NSString *)groupId success:(void (^)(QXAuthUserServiceMarkAllAsReadResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetWatchListResults *) GetWatchList:(NSString *)before limit:(NSNumber *)limit;
- (void) getWatchList:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetWatchListResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAddToWatchListResults *) AddToWatchList:(NSString *)entryId groupId:(NSString *)groupId remindMode:(NSString *)remindMode;
- (void) addToWatchList:(NSString *)entryId groupId:(NSString *)groupId remindMode:(NSString *)remindMode success:(void (^)(QXAuthUserServiceAddToWatchListResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceStopWatchingResults *) StopWatching:(NSString *)entryId groupId:(NSString *)groupId;
- (void) stopWatching:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceStopWatchingResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ReadWatching:(NSString *)entryId groupId:(NSString *)groupId;
- (void) readWatching:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceRemindMeResults *) RemindMe;
- (void) remindMe:(void (^)(QXAuthUserServiceRemindMeResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceStartSmartRemindingResults *) StartSmartReminding:(NSString *)groupId watchItemId:(NSString *)watchItemId;
- (void) startSmartReminding:(NSString *)groupId watchItemId:(NSString *)watchItemId success:(void (^)(QXAuthUserServiceStartSmartRemindingResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceStopRemindingResults *) StopReminding:(NSString *)groupId watchItemId:(NSString *)watchItemId;
- (void) stopReminding:(NSString *)groupId watchItemId:(NSString *)watchItemId success:(void (^)(QXAuthUserServiceStopRemindingResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateLikeResults *) UpdateLike:(QXLikeInput *)input;
- (void) updateLike:(QXLikeInput *)input success:(void (^)(QXAuthUserServiceUpdateLikeResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetDraftListResults *) GetDraftList:(NSString *)before limit:(NSNumber *)limit;
- (void) getDraftList:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetDraftListResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetDraftResults *) GetDraft:(NSString *)entryId groupId:(NSString *)groupId;
- (void) getDraft:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetDraftResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DeleteDraft:(NSString *)entryId groupId:(NSString *)groupId;
- (void) deleteDraft:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ChooseMarkdownEditor;
- (void) chooseMarkdownEditor:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ChooseStyledEditor;
- (void) chooseStyledEditor:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetNewGroupResults *) GetNewGroup;
- (void) getNewGroup:(void (^)(QXAuthUserServiceGetNewGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupResults *) GetGroup:(NSString *)groupId;
- (void) getGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateGroupResults *) CreateGroup:(QXGroupInput *)input;
- (void) createGroup:(QXGroupInput *)input success:(void (^)(QXAuthUserServiceCreateGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateGroup:(QXGroupInput *)input;
- (void) updateGroup:(QXGroupInput *)input success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateGroupLogo:(NSString *)groupId logoURL:(NSString *)logoURL;
- (void) updateGroupLogo:(NSString *)groupId logoURL:(NSString *)logoURL success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DeleteGroup:(NSString *)groupId;
- (void) deleteGroup:(NSString *)groupId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupBySlugResults *) GetGroupBySlug:(NSString *)slug;
- (void) getGroupBySlug:(NSString *)slug success:(void (^)(QXAuthUserServiceGetGroupBySlugResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupsResults *) GetGroups:(NSString *)keyword;
- (void) getGroups:(NSString *)keyword success:(void (^)(QXAuthUserServiceGetGroupsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetPublicGroupsResults *) GetPublicGroups:(NSString *)keyword;
- (void) getPublicGroups:(NSString *)keyword success:(void (^)(QXAuthUserServiceGetPublicGroupsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) AddUserToGroup:(NSString *)groupId userId:(NSString *)userId;
- (void) addUserToGroup:(NSString *)groupId userId:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RemoveUserFromGroup:(NSString *)groupId userId:(NSString *)userId;
- (void) removeUserFromGroup:(NSString *)groupId userId:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetClassifiedGroupsResults *) GetClassifiedGroups;
- (void) getClassifiedGroups:(void (^)(QXAuthUserServiceGetClassifiedGroupsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) BulkUpdateTasksInGroup:(NSString *)groupId taskPwMap:(NSArray *)taskPwMap taskInputs:(NSArray *)taskInputs;
- (void) bulkUpdateTasksInGroup:(NSString *)groupId taskPwMap:(NSArray *)taskPwMap taskInputs:(NSArray *)taskInputs success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetAuthUserResults *) GetAuthUser;
- (void) getAuthUser:(void (^)(QXAuthUserServiceGetAuthUserResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOrgUsersResults *) GetOrgUsers:(NSString *)keyword startFullName:(NSString *)startFullName limit:(NSNumber *)limit;
- (void) getOrgUsers:(NSString *)keyword startFullName:(NSString *)startFullName limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetOrgUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupUsersResults *) GetGroupUsers:(NSString *)groupId keyword:(NSString *)keyword onlyFollowers:(BOOL)onlyFollowers startFullName:(NSString *)startFullName limit:(NSNumber *)limit;
- (void) getGroupUsers:(NSString *)groupId keyword:(NSString *)keyword onlyFollowers:(BOOL)onlyFollowers startFullName:(NSString *)startFullName limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetGroupUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetUserResults *) GetUser:(NSString *)userId;
- (void) getUser:(NSString *)userId success:(void (^)(QXAuthUserServiceGetUserResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) EnableUser:(NSString *)userId;
- (void) enableUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DisableUser:(NSString *)userId;
- (void) disableUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DeleteUser:(NSString *)userId;
- (void) deleteUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) PromoteToSuperUser:(NSString *)userId;
- (void) promoteToSuperUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DemoteFromSuperUser:(NSString *)userId;
- (void) demoteFromSuperUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) FollowUser:(NSString *)userId;
- (void) followUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UnfollowUser:(NSString *)userId;
- (void) unfollowUser:(NSString *)userId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyFollowingUsersResults *) GetMyFollowingUsers;
- (void) getMyFollowingUsers:(void (^)(QXAuthUserServiceGetMyFollowingUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetPanelStatusResults *) GetPanelStatus;
- (void) getPanelStatus:(void (^)(QXAuthUserServiceGetPanelStatusResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetUserPreferencesResults *) GetUserPreferences;
- (void) getUserPreferences:(void (^)(QXAuthUserServiceGetUserPreferencesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateUserPreferencesResults *) UpdateUserPreferences:(QXPreferencesInput *)input;
- (void) updateUserPreferences:(QXPreferencesInput *)input success:(void (^)(QXAuthUserServiceUpdateUserPreferencesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOrgEmbedUsersResults *) GetOrgEmbedUsers;
- (void) getOrgEmbedUsers:(void (^)(QXAuthUserServiceGetOrgEmbedUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetNonStandardGroupEmbedUsersResults *) GetNonStandardGroupEmbedUsers;
- (void) getNonStandardGroupEmbedUsers:(void (^)(QXAuthUserServiceGetNonStandardGroupEmbedUsersResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateUserProfile:(QXUserProfileInput *)input;
- (void) updateUserProfile:(QXUserProfileInput *)input success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) SetPreferredLanguages:(NSArray *)languageCodes;
- (void) setPreferredLanguages:(NSArray *)languageCodes success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyCountResults *) GetMyCount;
- (void) getMyCount:(void (^)(QXAuthUserServiceGetMyCountResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceReadEntryResults *) ReadEntry:(NSString *)entryId groupId:(NSString *)groupId;
- (void) readEntry:(NSString *)entryId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceReadEntryResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetJoinOrgInvitationsResults *) GetJoinOrgInvitations;
- (void) getJoinOrgInvitations:(void (^)(QXAuthUserServiceGetJoinOrgInvitationsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOrganizationResults *) GetOrganization:(NSString *)orgId;
- (void) getOrganization:(NSString *)orgId success:(void (^)(QXAuthUserServiceGetOrganizationResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOrganizationsResults *) GetOrganizations:(NSArray *)orgIds;
- (void) getOrganizations:(NSArray *)orgIds success:(void (^)(QXAuthUserServiceGetOrganizationsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyOrgsUnreadInfoResults *) GetMyOrgsUnreadInfo;
- (void) getMyOrgsUnreadInfo:(void (^)(QXAuthUserServiceGetMyOrgsUnreadInfoResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyJoinedOrganizationsResults *) GetMyJoinedOrganizations;
- (void) getMyJoinedOrganizations:(void (^)(QXAuthUserServiceGetMyJoinedOrganizationsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetCurrentOrganizationResults *) GetCurrentOrganization;
- (void) getCurrentOrganization:(void (^)(QXAuthUserServiceGetCurrentOrganizationResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceSearchOrganizationsResults *) SearchOrganizations:(NSString *)keyword;
- (void) searchOrganizations:(NSString *)keyword success:(void (^)(QXAuthUserServiceSearchOrganizationsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateOrganizationResults *) UpdateOrganization:(QXOrganizationInput *)input;
- (void) updateOrganization:(QXOrganizationInput *)input success:(void (^)(QXAuthUserServiceUpdateOrganizationResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) SwitchOrganization:(NSString *)orgId;
- (void) switchOrganization:(NSString *)orgId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) AcceptShareRequestByAdmin:(NSString *)requestId;
- (void) acceptShareRequestByAdmin:(NSString *)requestId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RejectShareRequestByAdmin:(NSString *)requestId;
- (void) rejectShareRequestByAdmin:(NSString *)requestId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) StartTrial;
- (void) startTrial:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOrgSettingsResults *) GetOrgSettings;
- (void) getOrgSettings:(void (^)(QXAuthUserServiceGetOrgSettingsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateOrgSettings:(QXOrgSettingsInput *)orgSettingInput;
- (void) updateOrgSettings:(QXOrgSettingsInput *)orgSettingInput success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCanCreateGroupResults *) CanCreateGroup;
- (void) canCreateGroup:(void (^)(QXAuthUserServiceCanCreateGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCanInvitePeopleResults *) CanInvitePeople;
- (void) canInvitePeople:(void (^)(QXAuthUserServiceCanInvitePeopleResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceInvitePeopleResults *) InvitePeople:(NSArray *)emails allowEmpty:(BOOL)allowEmpty skipInvalidEmail:(BOOL)skipInvalidEmail customMessage:(NSString *)customMessage;
- (void) invitePeople:(NSArray *)emails allowEmpty:(BOOL)allowEmpty skipInvalidEmail:(BOOL)skipInvalidEmail customMessage:(NSString *)customMessage success:(void (^)(QXAuthUserServiceInvitePeopleResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) CancelInvitation:(NSString *)email;
- (void) cancelInvitation:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ResendInvitation:(NSString *)email;
- (void) resendInvitation:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ChangeLocale:(NSString *)localeName;
- (void) changeLocale:(NSString *)localeName success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateGroupAdvancedToDoSettings:(NSString *)gId settings:(NSString *)settings;
- (void) updateGroupAdvancedToDoSettings:(NSString *)gId settings:(NSString *)settings success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateMailPreference:(QXMailPreferenceInput *)input;
- (void) updateMailPreference:(QXMailPreferenceInput *)input success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServicePrepareChangingEmailResults *) PrepareChangingEmail:(NSString *)newEmail;
- (void) prepareChangingEmail:(NSString *)newEmail success:(void (^)(QXAuthUserServicePrepareChangingEmailResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ConfirmChangingEmail:(NSString *)token;
- (void) confirmChangingEmail:(NSString *)token success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) UpdateAccount:(QXMemberAccountInput *)input;
- (void) updateAccount:(QXMemberAccountInput *)input success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceSendShareRequestResults *) SendShareRequest:(NSString *)groupId email:(NSString *)email;
- (void) sendShareRequest:(NSString *)groupId email:(NSString *)email success:(void (^)(QXAuthUserServiceSendShareRequestResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetShareRequestsResults *) GetShareRequests:(NSString *)groupId;
- (void) getShareRequests:(NSString *)groupId success:(void (^)(QXAuthUserServiceGetShareRequestsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) CancelShareRequest:(NSString *)requestId;
- (void) cancelShareRequest:(NSString *)requestId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) StopSharingGroup:(NSString *)requestId;
- (void) stopSharingGroup:(NSString *)requestId success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) DismissPresentationTip;
- (void) dismissPresentationTip:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetMyChatEntriesResults *) GetMyChatEntries:(NSString *)before limit:(NSNumber *)limit;
- (void) getMyChatEntries:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetMyChatEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetPrivateChatResults *) GetPrivateChat:(NSString *)conversationId searchKeyWords:(NSString *)searchKeyWords;
- (void) getPrivateChat:(NSString *)conversationId searchKeyWords:(NSString *)searchKeyWords success:(void (^)(QXAuthUserServiceGetPrivateChatResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateQortexSupportResults *) CreateQortexSupport:(QXQortexSupportInput *)input;
- (void) createQortexSupport:(QXQortexSupportInput *)input success:(void (^)(QXAuthUserServiceCreateQortexSupportResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCreateQortexSupportCommentResults *) CreateQortexSupportComment:(QXQortexSupportInput *)input;
- (void) createQortexSupportComment:(QXQortexSupportInput *)input success:(void (^)(QXAuthUserServiceCreateQortexSupportCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetQortexSupportResults *) GetQortexSupport:(NSString *)entryId;
- (void) getQortexSupport:(NSString *)entryId success:(void (^)(QXAuthUserServiceGetQortexSupportResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetQortexSupportCommentResults *) GetQortexSupportComment:(NSString *)entryId;
- (void) getQortexSupportComment:(NSString *)entryId success:(void (^)(QXAuthUserServiceGetQortexSupportCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateQortexSupportResults *) UpdateQortexSupport:(QXQortexSupportInput *)input;
- (void) updateQortexSupport:(QXQortexSupportInput *)input success:(void (^)(QXAuthUserServiceUpdateQortexSupportResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateQortexSupportCommentResults *) UpdateQortexSupportComment:(QXQortexSupportInput *)input;
- (void) updateQortexSupportComment:(QXQortexSupportInput *)input success:(void (^)(QXAuthUserServiceUpdateQortexSupportCommentResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetQortexSupportHelpLinkResults *) GetQortexSupportHelpLink:(NSString *)title;
- (void) getQortexSupportHelpLink:(NSString *)title success:(void (^)(QXAuthUserServiceGetQortexSupportHelpLinkResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceNewTaskResults *) NewTask:(NSString *)groupId;
- (void) newTask:(NSString *)groupId success:(void (^)(QXAuthUserServiceNewTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceEditTaskResults *) EditTask:(NSString *)groupId taskId:(NSString *)taskId;
- (void) editTask:(NSString *)groupId taskId:(NSString *)taskId success:(void (^)(QXAuthUserServiceEditTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetAdvancedTaskResults *) GetAdvancedTask:(NSString *)taskId;
- (void) getAdvancedTask:(NSString *)taskId success:(void (^)(QXAuthUserServiceGetAdvancedTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceClaimTaskResults *) ClaimTask:(NSString *)taskId groupId:(NSString *)groupId;
- (void) claimTask:(NSString *)taskId groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceClaimTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceUpdateTaskResults *) UpdateTask:(QXTaskInput *)input;
- (void) updateTask:(QXTaskInput *)input success:(void (^)(QXAuthUserServiceUpdateTaskResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetTasksForMeResults *) GetTasksForMe;
- (void) getTasksForMe:(void (^)(QXAuthUserServiceGetTasksForMeResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOpenTasksIMadeResults *) GetOpenTasksIMade;
- (void) getOpenTasksIMade:(void (^)(QXAuthUserServiceGetOpenTasksIMadeResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetClosedTasksIMadeResults *) GetClosedTasksIMade:(NSString *)before limit:(NSNumber *)limit;
- (void) getClosedTasksIMade:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetClosedTasksIMadeResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetOpenTasksIWorkedOnResults *) GetOpenTasksIWorkedOn;
- (void) getOpenTasksIWorkedOn:(void (^)(QXAuthUserServiceGetOpenTasksIWorkedOnResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetClosedTasksIWorkedOnResults *) GetClosedTasksIWorkedOn:(NSString *)before limit:(NSNumber *)limit;
- (void) getClosedTasksIWorkedOn:(NSString *)before limit:(NSNumber *)limit success:(void (^)(QXAuthUserServiceGetClosedTasksIWorkedOnResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceGetGroupAdvancedToDoSettingResults *) GetGroupAdvancedToDoSetting:(NSString *)gId;
- (void) getGroupAdvancedToDoSetting:(NSString *)gId success:(void (^)(QXAuthUserServiceGetGroupAdvancedToDoSettingResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenAdvancedToDosInGroupResults *) AllOpenAdvancedToDosInGroup:(NSString *)groupId;
- (void) allOpenAdvancedToDosInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenAdvancedToDosInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenAdvancedToDosGroupingByUserInGroupResults *) AllOpenAdvancedToDosGroupingByUserInGroup:(NSString *)groupId;
- (void) allOpenAdvancedToDosGroupingByUserInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenAdvancedToDosGroupingByUserInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenAdvancedToDosGroupingByStatusInGroupResults *) AllOpenAdvancedToDosGroupingByStatusInGroup:(NSString *)groupId;
- (void) allOpenAdvancedToDosGroupingByStatusInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenAdvancedToDosGroupingByStatusInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenAdvancedToDosGroupingByLabelInGroupResults *) AllOpenAdvancedToDosGroupingByLabelInGroup:(NSString *)groupId;
- (void) allOpenAdvancedToDosGroupingByLabelInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenAdvancedToDosGroupingByLabelInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenBasicToDosInGroupResults *) AllOpenBasicToDosInGroup:(NSString *)groupId;
- (void) allOpenBasicToDosInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenBasicToDosInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllOpenBasicToDosGroupingByUserInGroupResults *) AllOpenBasicToDosGroupingByUserInGroup:(NSString *)groupId;
- (void) allOpenBasicToDosGroupingByUserInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllOpenBasicToDosGroupingByUserInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllClosedBasicToDosInGroupResults *) AllClosedBasicToDosInGroup:(NSString *)groupId afterTimeS:(NSString *)afterTimeS;
- (void) allClosedBasicToDosInGroup:(NSString *)groupId afterTimeS:(NSString *)afterTimeS success:(void (^)(QXAuthUserServiceAllClosedBasicToDosInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceAllClosedAdvancedToDosInGroupResults *) AllClosedAdvancedToDosInGroup:(NSString *)groupId;
- (void) allClosedAdvancedToDosInGroup:(NSString *)groupId success:(void (^)(QXAuthUserServiceAllClosedAdvancedToDosInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceMoreClosedAdvancedToDosWithStatusInGroupResults *) MoreClosedAdvancedToDosWithStatusInGroup:(NSString *)groupId status:(NSNumber *)status afterTime:(NSString *)afterTime;
- (void) moreClosedAdvancedToDosWithStatusInGroup:(NSString *)groupId status:(NSNumber *)status afterTime:(NSString *)afterTime success:(void (^)(QXAuthUserServiceMoreClosedAdvancedToDosWithStatusInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceCountOfClosedToDosInGroupResults *) CountOfClosedToDosInGroup:(NSNumber *)ttype groupId:(NSString *)groupId;
- (void) countOfClosedToDosInGroup:(NSNumber *)ttype groupId:(NSString *)groupId success:(void (^)(QXAuthUserServiceCountOfClosedToDosInGroupResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserServiceToDoCSVResults *) ToDoCSV:(NSString *)groupId;
- (void) toDoCSV:(NSString *)groupId success:(void (^)(QXAuthUserServiceToDoCSVResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;
@end


// --- QXGetSessionParams ---
@interface QXPublicServiceGetSessionParams : NSObject

@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSString * Password;
@property (nonatomic, strong) NSString * Locale;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetSessionResults ---
@interface QXPublicServiceGetSessionResults : NSObject

@property (nonatomic, strong) NSString * Session;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAuthorizedAdminParams ---
@interface QXPublicServiceGetAuthorizedAdminParams : NSObject

@property (nonatomic, strong) NSString * Session;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetAuthorizedAdminResults ---
@interface QXPublicServiceGetAuthorizedAdminResults : NSObject

@property (nonatomic, strong) QXEmbedUser * ApiEmbedUser;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXFindPasswordParams ---
@interface QXPublicServiceFindPasswordParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXFindPasswordResults ---
@interface QXPublicServiceFindPasswordResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResetPasswordParams ---
@interface QXPublicServiceResetPasswordParams : NSObject

@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * Password;
@property (nonatomic, strong) NSString * ConfirmedPassword;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXResetPasswordResults ---
@interface QXPublicServiceResetPasswordResults : NSObject

@property (nonatomic, strong) NSString * MemberId;
@property (nonatomic, strong) NSString * Email;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPrepareChangingEmailParams ---
@interface QXPublicServicePrepareChangingEmailParams : NSObject

@property (nonatomic, strong) NSString * MemberId;
@property (nonatomic, strong) NSString * NewEmail;
@property (nonatomic, strong) NSString * SharingToken;
@property (nonatomic, strong) NSString * InvitationToken;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXPrepareChangingEmailResults ---
@interface QXPublicServicePrepareChangingEmailResults : NSObject

@property (nonatomic, strong) QXEmailChanger * Changer;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXConfirmChangingEmailParams ---
@interface QXPublicServiceConfirmChangingEmailParams : NSObject

@property (nonatomic, strong) NSString * Token;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXConfirmChangingEmailResults ---
@interface QXPublicServiceConfirmChangingEmailResults : NSObject

@property (nonatomic, strong) NSString * ActivationToken;
@property (nonatomic, strong) NSString * SharingToken;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelChangingEmailParams ---
@interface QXPublicServiceCancelChangingEmailParams : NSObject

@property (nonatomic, strong) NSString * Token;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCancelChangingEmailResults ---
@interface QXPublicServiceCancelChangingEmailResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChangeEmailToAcceptSharingParams ---
@interface QXPublicServiceChangeEmailToAcceptSharingParams : NSObject

@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * NewEmail;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXChangeEmailToAcceptSharingResults ---
@interface QXPublicServiceChangeEmailToAcceptSharingResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestParams ---
@interface QXPublicServiceGetShareRequestParams : NSObject

@property (nonatomic, strong) NSString * Token;
@property (nonatomic, strong) NSString * MemberId;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetShareRequestResults ---
@interface QXPublicServiceGetShareRequestResults : NSObject

@property (nonatomic, strong) QXShareRequest * ShareRequest;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXContactUsParams ---
@interface QXPublicServiceContactUsParams : NSObject

@property (nonatomic, strong) QXContactInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXContactUsResults ---
@interface QXPublicServiceContactUsResults : NSObject

@property (nonatomic, strong) QXContactInfo * Contact;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetBlogEntriesParams ---
@interface QXPublicServiceGetBlogEntriesParams : NSObject

@property (nonatomic, strong) NSString * Doi;
@property (nonatomic, strong) NSNumber * PageNum;
@property (nonatomic, strong) NSNumber * Limit;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetBlogEntriesResults ---
@interface QXPublicServiceGetBlogEntriesResults : NSObject

@property (nonatomic, strong) QXBlog * Blog;
@property (nonatomic, strong) NSArray * BlogEntries;
@property (nonatomic, strong) NSNumber * TotalPageNum;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetBlogEntryBySlugParams ---
@interface QXPublicServiceGetBlogEntryBySlugParams : NSObject

@property (nonatomic, strong) NSString * Doi;
@property (nonatomic, strong) NSString * Slug;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGetBlogEntryBySlugResults ---
@interface QXPublicServiceGetBlogEntryBySlugResults : NSObject

@property (nonatomic, strong) QXBlog * Blog;
@property (nonatomic, strong) QXBlogEntry * BlogEntry;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGenerateBlogEntrySlugParams ---
@interface QXPublicServiceGenerateBlogEntrySlugParams : NSObject

@property (nonatomic, strong) NSString * Doi;
@property (nonatomic, strong) NSString * Slug;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXGenerateBlogEntrySlugResults ---
@interface QXPublicServiceGenerateBlogEntrySlugResults : NSObject

@property (nonatomic, strong) NSString * ValidSlug;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateNewsletterParams ---
@interface QXPublicServiceCreateNewsletterParams : NSObject

@property (nonatomic, strong) QXNewsletterInput * Input;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXCreateNewsletterResults ---
@interface QXPublicServiceCreateNewsletterResults : NSObject

@property (nonatomic, strong) QXNewsletter * Newsletter;
@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewSignupTokenParams ---
@interface QXPublicServiceRequestNewSignupTokenParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewSignupTokenResults ---
@interface QXPublicServiceRequestNewSignupTokenResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewInvitationTokenParams ---
@interface QXPublicServiceRequestNewInvitationTokenParams : NSObject

@property (nonatomic, strong) NSString * OrgId;
@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewInvitationTokenResults ---
@interface QXPublicServiceRequestNewInvitationTokenResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewSharingTokenParams ---
@interface QXPublicServiceRequestNewSharingTokenParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestNewSharingTokenResults ---
@interface QXPublicServiceRequestNewSharingTokenResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXInviteMeParams ---
@interface QXPublicServiceInviteMeParams : NSObject

@property (nonatomic, strong) NSString * OrganizationId;
@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXInviteMeResults ---
@interface QXPublicServiceInviteMeResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestSignupParams ---
@interface QXPublicServiceRequestSignupParams : NSObject

@property (nonatomic, strong) NSString * Email;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end

// --- QXRequestSignupResults ---
@interface QXPublicServiceRequestSignupResults : NSObject

@property (nonatomic, strong) NSError * Err;

- (id) initWithDictionary:(NSDictionary*)dict;
- (NSDictionary*) dictionary;

@end


@interface QXPublicService : NSObject- (NSDictionary*) dictionary;

- (QXPublicServiceGetSessionResults *) GetSession:(NSString *)email password:(NSString *)password locale:(NSString *)locale;
- (void) getSession:(NSString *)email password:(NSString *)password locale:(NSString *)locale success:(void (^)(QXPublicServiceGetSessionResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthUserService *) GetAuthUserService:(NSString *)session;
- (void) getAuthUserService:(NSString *)session success:(void (^)(QXAuthUserService* AuthUserService))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceGetAuthorizedAdminResults *) GetAuthorizedAdmin:(NSString *)session;
- (void) getAuthorizedAdmin:(NSString *)session success:(void (^)(QXPublicServiceGetAuthorizedAdminResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXAuthAdminService *) GetAuthAdminService:(NSString *)session;
- (void) getAuthAdminService:(NSString *)session success:(void (^)(QXAuthAdminService* AuthAdminService))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) FindPassword:(NSString *)email;
- (void) findPassword:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceResetPasswordResults *) ResetPassword:(NSString *)token password:(NSString *)password confirmedPassword:(NSString *)confirmedPassword;
- (void) resetPassword:(NSString *)token password:(NSString *)password confirmedPassword:(NSString *)confirmedPassword success:(void (^)(QXPublicServiceResetPasswordResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServicePrepareChangingEmailResults *) PrepareChangingEmail:(NSString *)memberId newEmail:(NSString *)newEmail sharingToken:(NSString *)sharingToken invitationToken:(NSString *)invitationToken;
- (void) prepareChangingEmail:(NSString *)memberId newEmail:(NSString *)newEmail sharingToken:(NSString *)sharingToken invitationToken:(NSString *)invitationToken success:(void (^)(QXPublicServicePrepareChangingEmailResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceConfirmChangingEmailResults *) ConfirmChangingEmail:(NSString *)token;
- (void) confirmChangingEmail:(NSString *)token success:(void (^)(QXPublicServiceConfirmChangingEmailResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) CancelChangingEmail:(NSString *)token;
- (void) cancelChangingEmail:(NSString *)token success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) ChangeEmailToAcceptSharing:(NSString *)token newEmail:(NSString *)newEmail;
- (void) changeEmailToAcceptSharing:(NSString *)token newEmail:(NSString *)newEmail success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceGetShareRequestResults *) GetShareRequest:(NSString *)token memberId:(NSString *)memberId;
- (void) getShareRequest:(NSString *)token memberId:(NSString *)memberId success:(void (^)(QXPublicServiceGetShareRequestResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceContactUsResults *) ContactUs:(QXContactInput *)input;
- (void) contactUs:(QXContactInput *)input success:(void (^)(QXPublicServiceContactUsResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceGetBlogEntriesResults *) GetBlogEntries:(NSString *)doi pageNum:(NSNumber *)pageNum limit:(NSNumber *)limit;
- (void) getBlogEntries:(NSString *)doi pageNum:(NSNumber *)pageNum limit:(NSNumber *)limit success:(void (^)(QXPublicServiceGetBlogEntriesResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceGetBlogEntryBySlugResults *) GetBlogEntryBySlug:(NSString *)doi slug:(NSString *)slug;
- (void) getBlogEntryBySlug:(NSString *)doi slug:(NSString *)slug success:(void (^)(QXPublicServiceGetBlogEntryBySlugResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceGenerateBlogEntrySlugResults *) GenerateBlogEntrySlug:(NSString *)doi slug:(NSString *)slug;
- (void) generateBlogEntrySlug:(NSString *)doi slug:(NSString *)slug success:(void (^)(QXPublicServiceGenerateBlogEntrySlugResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (QXPublicServiceCreateNewsletterResults *) CreateNewsletter:(QXNewsletterInput *)input;
- (void) createNewsletter:(QXNewsletterInput *)input success:(void (^)(QXPublicServiceCreateNewsletterResults *results))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RequestNewSignupToken:(NSString *)email;
- (void) requestNewSignupToken:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RequestNewInvitationToken:(NSString *)orgId email:(NSString *)email;
- (void) requestNewInvitationToken:(NSString *)orgId email:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RequestNewSharingToken:(NSString *)email;
- (void) requestNewSharingToken:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) InviteMe:(NSString *)organizationId email:(NSString *)email;
- (void) inviteMe:(NSString *)organizationId email:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;

- (NSError *) RequestSignup:(NSString *)email;
- (void) requestSignup:(NSString *)email success:(void (^)(NSError *error))successBlock failure:(void (^)(NSError *error))failureBlock;
@end

